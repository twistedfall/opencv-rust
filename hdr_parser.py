...
class CppHeaderParser(object):
    ...
    def parse_stmt(self, stmt, end_token, use_umat=False, docstring=""):
        ...
        if (context == "struct" or context == "class") and end_token == ";" and stmt:
            ...
            if "CV_PROP" in stmt or class_decl[0] in ("class cv.Mat", "class cv.RotatedRect", "class cv.TermCriteria", "class cv.Range"): # or (class_decl and ("/Map" in class_decl[2])):
                var_modlist = []
    ...
    def parse(self, hname, wmode=True):
        ...
        for l0 in linelist:
            ...
            if state == DIRECTIVE:
                ...
                if define:
                    decls.append(["const " + define.group(1), define.group(2), [], [], docstring])
                continue
