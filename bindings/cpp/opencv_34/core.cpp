...
extern "C" {
	...
	Result_void cv_ipp_setUseIPP_NE_bool(bool flag) {
		try {
	...
	Result_void cv_Algorithm_write_const_const_Ptr_FileStorage_X_const_StringX(void* instance, void* fs, const char* name) {
		try {
			reinterpret_cast<cv::Algorithm*>(instance)->write(*reinterpret_cast<const cv::Ptr<cv::FileStorage>*>(fs), cv::String(name));
			return Ok();
		} OCVRS_CATCH(Result_void)
	}
	...
	Result<void*> cv_FileNodeIterator_readRaw_const_StringX_unsigned_charX_size_t(void* instance, const char* fmt, unsigned char* vec, size_t len) {
		try {
			cv::FileNodeIterator ret = reinterpret_cast<cv::FileNodeIterator*>(instance)->readRaw(cv::String(fmt), vec, len);
			return Ok<void*>(new cv::FileNodeIterator(ret));
		} OCVRS_CATCH(Result<void*>)
	}
	...
	Result<void*> cv__InputArray__InputArray_const_vector_vector_bool__X(void* unnamed) {
		try {
			cv::_InputArray* ret = new cv::_InputArray(*reinterpret_cast<const std::vector<std::vector<bool>>*>(unnamed));
			return Ok<void*>(ret);
		} OCVRS_CATCH(Result<void*>)
	}
	...
	Result<void*> cv__OutputArray__OutputArray_vector_vector_bool__X(void* unnamed) {
		try {
			cv::_OutputArray* ret = new cv::_OutputArray(*reinterpret_cast<std::vector<std::vector<bool>>*>(unnamed));
			return Ok<void*>(ret);
		} OCVRS_CATCH(Result<void*>)
	}
	...
	Result<bool> cv_ocl_Device_isExtensionSupported_const_const_StringX(void* instance, const char* extensionName) {
		try {
			bool ret = reinterpret_cast<cv::ocl::Device*>(instance)->isExtensionSupported(cv::String(extensionName));
			return Ok<bool>(ret);
		} OCVRS_CATCH(Result<bool>)
	}
