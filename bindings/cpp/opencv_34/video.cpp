...
extern "C" {
	...
	Result<cv::Ptr<cv::DualTVL1OpticalFlow>*> cv_createOptFlow_DualTVL1() {
		...
		} OCVRS_CATCH(Result<cv::Ptr<cv::DualTVL1OpticalFlow>*>)
	}
	...
	Result<cv::Mat*> cv_estimateRigidTransform_const__InputArrayX_const__InputArrayX_bool_int_double_int(const cv::_InputArray* src, const cv::_InputArray* dst, bool fullAffine, int ransacMaxIters, double ransacGoodRatio, int ransacSize0) {
		try {
