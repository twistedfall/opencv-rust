...
extern "C" {
	...
	Result<cv::Rect> cv_boundingRect_const__InputArrayX(const cv::_InputArray* points) {
		try {
	...
	Result_void cv_demosaicing_const__InputArrayX_const__OutputArrayX_int_int(const cv::_InputArray* _src, const cv::_OutputArray* _dst, int code, int dcn) {
		try {
	...
	Result_void cv_drawMarker_MatX_Point_const_ScalarX_int_int_int_int(cv::Mat* img, const cv::Point* position, const cv::Scalar* color, int markerType, int markerSize, int thickness, int line_type) {
		try {
	...
	Result_void cv_findContours_const__InputOutputArrayX_const__OutputArrayX_const__OutputArrayX_int_int_Point(const cv::_InputOutputArray* image, const cv::_OutputArray* contours, const cv::_OutputArray* hierarchy, int mode, int method, const cv::Point* offset) {
		try {
	...
	Result_void cv_findContours_const__InputOutputArrayX_const__OutputArrayX_int_int_Point(const cv::_InputOutputArray* image, const cv::_OutputArray* contours, int mode, int method, const cv::Point* offset) {
		try {
	...
	Result<cv::Mat*> cv_getPerspectiveTransform_const__InputArrayX_const__InputArrayX(const cv::_InputArray* src, const cv::_InputArray* dst) {
		try {
			cv::Mat ret = cv::getPerspectiveTransform(*src, *dst);
			return Ok(new cv::Mat(ret));
		} OCVRS_CATCH(OCVRS_TYPE(Result<cv::Mat*>))
	...
	Result_void cv_minEnclosingCircle_const__InputArrayX_Point2fX_floatR(const cv::_InputArray* points, cv::Point2f* center, float* radius) {
		try {
	...
	Result_void cv_rectangle_MatX_Rect_const_ScalarX_int_int_int(cv::Mat* img, const cv::Rect* rec, const cv::Scalar* color, int thickness, int lineType, int shift) {
		try {
	...
	Result_void cv_sqrBoxFilter_const__InputArrayX_const__OutputArrayX_int_Size_Point_bool_int(const cv::_InputArray* _src, const cv::_OutputArray* _dst, int ddepth, const cv::Size* ksize, const cv::Point* anchor, bool normalize, int borderType) {
		try {
