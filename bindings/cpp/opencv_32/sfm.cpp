...
extern "C" {
	...
	Result_void cv_sfm_BaseSFM_run_const_vector_string_X(cv::sfm::BaseSFM* instance, const std::vector<std::string>* images) {
		try {
			instance->run(*images);
			return Ok();
		} OCVRS_CATCH(Result_void)
	...
	Result_void cv_sfm_SFMLibmvEuclideanReconstruction_run_const_vector_string_X(cv::sfm::SFMLibmvEuclideanReconstruction* instance, const std::vector<std::string>* images) {
		try {
			instance->run(*images);
			return Ok();
		} OCVRS_CATCH(Result_void)
