...
template struct Result<cv::Ptr<cv::saliency::Saliency>*>;
...
extern "C" {
	void cv_PtrOfSaliency_delete(cv::Ptr<cv::saliency::Saliency>* instance) {
		delete instance;
	}

	const cv::saliency::Saliency* cv_PtrOfSaliency_get_inner_ptr(const cv::Ptr<cv::saliency::Saliency>* instance) {
		return instance->get();
	}

	cv::saliency::Saliency* cv_PtrOfSaliency_get_inner_ptr_mut(cv::Ptr<cv::saliency::Saliency>* instance) {
		return instance->get();
	}
}
