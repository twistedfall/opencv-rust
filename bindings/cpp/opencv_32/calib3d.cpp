...
extern "C" {
	...
	Result<cv::Mat*> cv_findFundamentalMat_const__InputArrayR_const__InputArrayR_const__OutputArrayR_int_double_double(const cv::_InputArray* points1, const cv::_InputArray* points2, const cv::_OutputArray* mask, int method, double param1, double param2) {
		try {
			cv::Mat ret = cv::findFundamentalMat(*points1, *points2, *mask, method, param1, param2);
			return Ok(new cv::Mat(ret));
		} OCVRS_CATCH(OCVRS_TYPE(Result<cv::Mat*>))
	}
	
	Result<cv::Mat*> cv_findFundamentalMat_const__InputArrayR_const__InputArrayR_int_double_double_const__OutputArrayR(const cv::_InputArray* points1, const cv::_InputArray* points2, int method, double param1, double param2, const cv::_OutputArray* mask) {
		try {
			cv::Mat ret = cv::findFundamentalMat(*points1, *points2, method, param1, param2, *mask);
			return Ok(new cv::Mat(ret));
		} OCVRS_CATCH(OCVRS_TYPE(Result<cv::Mat*>))
	}
