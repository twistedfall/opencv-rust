...
extern "C" {
	...
	Result<cv::Point2f> cv_detail_AffineWarper_warpPoint_const_Point2fX_const__InputArrayX_const__InputArrayX(void* instance, const cv::Point2f* pt, void* K, void* R) {
		try {
			cv::Point2f ret = reinterpret_cast<cv::detail::AffineWarper*>(instance)->warpPoint(*pt, *reinterpret_cast<const cv::_InputArray*>(K), *reinterpret_cast<const cv::_InputArray*>(R));
	...
	Result<cv::Point> cv_detail_AffineWarper_warp_const__InputArrayX_const__InputArrayX_const__InputArrayX_int_int_const__OutputArrayX(void* instance, void* src, void* K, void* R, int interp_mode, int border_mode, void* dst) {
		try {
			cv::Point ret = reinterpret_cast<cv::detail::AffineWarper*>(instance)->warp(*reinterpret_cast<const cv::_InputArray*>(src), *reinterpret_cast<const cv::_InputArray*>(K), *reinterpret_cast<const cv::_InputArray*>(R), interp_mode, border_mode, *reinterpret_cast<const cv::_OutputArray*>(dst));
			return Ok<cv::Point>(ret);
		} OCVRS_CATCH(Result<cv::Point>)
	}
