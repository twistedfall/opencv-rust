...
extern "C" {
	...
	Result<bool> cv_imencode_const_StringX_const__InputArrayX_vector_unsigned_char_X_const_vector_int_X(const char* ext, void* img, void* buf, void* params) {
		try {
			bool ret = cv::imencode(cv::String(ext), *reinterpret_cast<const cv::_InputArray*>(img), *reinterpret_cast<std::vector<unsigned char>*>(buf), *reinterpret_cast<const std::vector<int>*>(params));
			return Ok<bool>(ret);
	...
	Result<bool> cv_imreadmulti_const_StringX_vector_Mat_X_int(const char* filename, void* mats, int flags) {
		try {
			bool ret = cv::imreadmulti(cv::String(filename), *reinterpret_cast<std::vector<cv::Mat>*>(mats), flags);
			return Ok<bool>(ret);
		} OCVRS_CATCH(Result<bool>)
	}
	...
	Result<bool> cv_imwrite_const_StringX_const__InputArrayX_const_vector_int_X(const char* filename, void* img, void* params) {
		try {
			bool ret = cv::imwrite(cv::String(filename), *reinterpret_cast<const cv::_InputArray*>(img), *reinterpret_cast<const std::vector<int>*>(params));
			return Ok<bool>(ret);
		} OCVRS_CATCH(Result<bool>)
	}
