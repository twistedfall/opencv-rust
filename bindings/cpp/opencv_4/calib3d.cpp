...
extern "C" {
	...
	Result<int> cv_LMSolver_run_const_const__InputOutputArrayX(void* instance, void* param) {
		try {
			int ret = reinterpret_cast<cv::LMSolver*>(instance)->run(*reinterpret_cast<const cv::_InputOutputArray*>(param));
			return Ok<int>(ret);
		} OCVRS_CATCH(Result<int>)
	}
	
	Result_void cv_LMSolver_setMaxIters_int(void* instance, int maxIters) {
		try {
	...
	Result<int> cv_LMSolver_getMaxIters_const(void* instance) {
		try {
	...
	Result<void*> cv_LMSolver_create_const_Ptr_Callback_X_int(void* cb, int maxIters) {
		try {
			cv::Ptr<cv::LMSolver> ret = cv::LMSolver::create(*reinterpret_cast<const cv::Ptr<cv::LMSolver::Callback>*>(cb), maxIters);
			return Ok<void*>(new cv::Ptr<cv::LMSolver>(ret));
		} OCVRS_CATCH(Result<void*>)
	}
	
	Result<void*> cv_LMSolver_create_const_Ptr_Callback_X_int_double(void* cb, int maxIters, double eps) {
		try {
			cv::Ptr<cv::LMSolver> ret = cv::LMSolver::create(*reinterpret_cast<const cv::Ptr<cv::LMSolver::Callback>*>(cb), maxIters, eps);
			return Ok<void*>(new cv::Ptr<cv::LMSolver>(ret));
		} OCVRS_CATCH(Result<void*>)
	}
	
	Result<bool> cv_LMSolver_Callback_compute_const_const__InputArrayX_const__OutputArrayX_const__OutputArrayX(void* instance, void* param, void* err, void* J) {
		try {
			bool ret = reinterpret_cast<cv::LMSolver::Callback*>(instance)->compute(*reinterpret_cast<const cv::_InputArray*>(param), *reinterpret_cast<const cv::_OutputArray*>(err), *reinterpret_cast<const cv::_OutputArray*>(J));
			return Ok<bool>(ret);
		} OCVRS_CATCH(Result<bool>)
	}
