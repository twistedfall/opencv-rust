...
pub fn create_plot2d(data: &dyn core::ToInputArray) -> Result<types::PtrOfPlot2d> {
	input_array_arg!(data);
	unsafe { sys::cv_plot_createPlot2d_const__InputArrayX(data.as_raw__InputArray()) }.into_result().map(|ptr| types::PtrOfPlot2d { ptr })
}
...
pub fn create_plot2d_1(data_x: &dyn core::ToInputArray, data_y: &dyn core::ToInputArray) -> Result<types::PtrOfPlot2d> {
	input_array_arg!(data_x);
	input_array_arg!(data_y);
	unsafe { sys::cv_plot_createPlot2d_const__InputArrayX_const__InputArrayX(data_x.as_raw__InputArray(), data_y.as_raw__InputArray()) }.into_result().map(|ptr| types::PtrOfPlot2d { ptr })
}
...
pub trait Plot2d: core::AlgorithmTrait {
	...
	fn render(&mut self, _plot_result: &mut dyn core::ToOutputArray) -> Result<()> {
		...
		unsafe { sys::cv_plot_Plot2d_render_const__OutputArrayX(self.as_raw_Plot2d(), _plot_result.as_raw__OutputArray()) }.into_result()
	}
