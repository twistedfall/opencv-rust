...
impl BoostDesc {
	#[inline] pub fn as_raw_BoostDesc(&self) -> *const c_void { self.as_raw() }
	#[inline] pub fn as_raw_mut_BoostDesc(&mut self) -> *mut c_void { self.as_raw_mut() }
}
...
pub trait VGG: crate::features2d::Feature2DTrait {
	...
	fn compute(&mut self, image: &dyn core::ToInputArray, keypoints: &mut core::Vector::<core::KeyPoint>, descriptors: &mut dyn core::ToOutputArray) -> Result<()> {
		input_array_arg!(image);
		output_array_arg!(descriptors);
		unsafe { sys::cv_xfeatures2d_VGG_compute_const__InputArrayR_vector_KeyPoint_R_const__OutputArrayR(self.as_raw_mut_VGG(), image.as_raw__InputArray(), keypoints.as_raw_mut_VectorOfKeyPoint(), descriptors.as_raw__OutputArray()) }.into_result()
	}
