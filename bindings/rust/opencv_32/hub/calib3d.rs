...
//! 
//! ![Pinhole camera model](https://docs.opencv.org/3.2.0/pinhole_camera_model.png)
//! 
...
//! 
//! The next figure shows two common types of radial distortion: barrel distortion (typically ![inline formula](https://latex.codecogs.com/png.latex?%20k%5F1%20%3E%200%20) and pincushion distortion (typically ![inline formula](https://latex.codecogs.com/png.latex?%20k%5F1%20%3C%200%20)).
//! 
...
//! ![inline formula](https://latex.codecogs.com/png.latex?y%27%27). This distortion can be modelled in the following way, see e.g. [Louhichi07](https://docs.opencv.org/3.2.0/d0/de3/citelist.html#CITEREF_Louhichi07).
//! 
...
/// object, eg. see [Slabaugh](https://docs.opencv.org/3.2.0/d0/de3/citelist.html#CITEREF_Slabaugh) . Returned tree rotation matrices and corresponding three Euler angules
...
/// *   **CV_CALIB_ZERO_TANGENT_DIST** Tangential distortion coefficients ![inline formula](https://latex.codecogs.com/png.latex?%28p%5F1%2C%20p%5F2%29) are set
...
/// views. The algorithm is based on [Zhang2000](https://docs.opencv.org/3.2.0/d0/de3/citelist.html#CITEREF_Zhang2000) and [BouguetMCT](https://docs.opencv.org/3.2.0/d0/de3/citelist.html#CITEREF_BouguetMCT) . The coordinates of 3D object
...
/// 
/// This function decompose an essential matrix E using svd decomposition [HartleyZ00](https://docs.opencv.org/3.2.0/d0/de3/citelist.html#CITEREF_HartleyZ00) . Generally 4
...
/// must be in front of the camera). The decomposition method is described in detail in [Malis](https://docs.opencv.org/3.2.0/d0/de3/citelist.html#CITEREF_Malis) .
...
/// principal axes that results in the same orientation of an object, eg. see [Slabaugh](https://docs.opencv.org/3.2.0/d0/de3/citelist.html#CITEREF_Slabaugh) . Returned
...
/// * method: Robust method used to compute tranformation. The following methods are possible:
...
/// * out: Output 3D affine transformation matrix ![inline formula](https://latex.codecogs.com/png.latex?3%20%5Ctimes%204) .
/// * inliers: Output vector indicating which points are inliers.
...
/// ![block formula](https://latex.codecogs.com/png.latex?%20%5Cbegin%7Bbmatrix%7D%20%5Ccos%28%5Ctheta%29s%20%26%20%2D%5Csin%28%5Ctheta%29s%20%26%20tx%20%5C%5C%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%5Csin%28%5Ctheta%29s%20%26%20%5Ccos%28%5Ctheta%29s%20%26%20ty%0A%5Cend%7Bbmatrix%7D%20)
/// Where ![inline formula](https://latex.codecogs.com/png.latex?%20%5Ctheta%20) is the rotation angle, ![inline formula](https://latex.codecogs.com/png.latex?%20s%20) the scaling factor and ![inline formula](https://latex.codecogs.com/png.latex?%20tx%2C%20ty%20) are
...
/// 
/// This function estimates essential matrix based on the five-point algorithm solver in [Nister03](https://docs.opencv.org/3.2.0/d0/de3/citelist.html#CITEREF_Nister03) .
/// [SteweniusCFS](https://docs.opencv.org/3.2.0/d0/de3/citelist.html#CITEREF_SteweniusCFS) is also a related. The epipolar geometry is described by the following equation:
/// 
...
/// * param1: Parameter used for RANSAC. It is the maximum distance from a point to an epipolar
/// line in pixels, beyond which the point is considered an outlier and is not used for computing the
/// final fundamental matrix. It can be set to something like 1-3, depending on the accuracy of the
/// point localization, image resolution, and the image noise.
/// * param2: Parameter used for the RANSAC or LMedS methods only. It specifies a desirable level
...
/// * param1: 3.
/// * param2: 0.99
pub fn find_fundamental_mat_mask(points1: &dyn core::ToInputArray, points2: &dyn core::ToInputArray, mask: &mut dyn core::ToOutputArray, method: i32, param1: f64, param2: f64) -> Result<core::Mat> {
	input_array_arg!(points1);
	input_array_arg!(points2);
	output_array_arg!(mask);
	unsafe { sys::cv_findFundamentalMat_const__InputArrayX_const__InputArrayX_const__OutputArrayX_int_double_double(points1.as_raw__InputArray(), points2.as_raw__InputArray(), mask.as_raw__OutputArray(), method, param1, param2) }.into_result().map(|ptr| core::Mat { ptr })
}
...
/// * param2: 0.99
/// * mask: noArray()
pub fn find_fundamental_mat(points1: &dyn core::ToInputArray, points2: &dyn core::ToInputArray, method: i32, param1: f64, param2: f64, mask: &mut dyn core::ToOutputArray) -> Result<core::Mat> {
	input_array_arg!(points1);
	input_array_arg!(points2);
	output_array_arg!(mask);
	unsafe { sys::cv_findFundamentalMat_const__InputArrayX_const__InputArrayX_int_double_double_const__OutputArrayX(points1.as_raw__InputArray(), points2.as_raw__InputArray(), method, param1, param2, mask.as_raw__OutputArray()) }.into_result().map(|ptr| core::Mat { ptr })
}
...
/// ![block formula](https://latex.codecogs.com/png.latex?%5C%7C%20%5Ctexttt%7BdstPoints%7D%20%5Fi%20%2D%20%20%5Ctexttt%7BconvertPointsHomogeneous%7D%20%28%20%5Ctexttt%7BH%7D%20%2A%20%5Ctexttt%7BsrcPoints%7D%20%5Fi%29%20%5C%7C%20%20%3E%20%20%5Ctexttt%7BransacReprojThreshold%7D)
/// then the point ![inline formula](https://latex.codecogs.com/png.latex?i) is considered an outlier. If srcPoints and dstPoints are measured in pixels,
...
/// determined up to a scale. Thus, it is normalized so that ![inline formula](https://latex.codecogs.com/png.latex?h%5F%7B33%7D%3D1). Note that whenever an H matrix
...
/// * flags: Operation flags that may be zero or CV_CALIB_ZERO_DISPARITY . If the flag is set,
...
/// 
/// ![image](https://docs.opencv.org/3.2.0/fisheye_undistorted.jpg)
/// 
/// ## C++ default parameters
/// * knew: cv::noArray()
/// * new_size: Size()
...
/// triangulated 3D points should have positive depth. Some details can be found in [Nister03](https://docs.opencv.org/3.2.0/d0/de3/citelist.html#CITEREF_Nister03) .
/// 
...
/// ![block formula](https://latex.codecogs.com/png.latex?sd%28%20%5Ctexttt%7Bpt1%7D%20%2C%20%5Ctexttt%7Bpt2%7D%20%29%3D%20%5Cfrac%7B%28%5Ctexttt%7Bpt2%7D%5Et%20%5Ccdot%20%5Ctexttt%7BF%7D%20%5Ccdot%20%5Ctexttt%7Bpt1%7D%29%5E2%7D%7B%28%5Ctexttt%7BF%7D%20%5Ccdot%20%5Ctexttt%7Bpt1%7D%29%280%29%20%2B%20%28%5Ctexttt%7BF%7D%20%5Ccdot%20%5Ctexttt%7Bpt1%7D%29%281%29%20%2B%20%28%5Ctexttt%7BF%7D%5Et%20%5Ccdot%20%5Ctexttt%7Bpt2%7D%29%280%29%20%2B%20%28%5Ctexttt%7BF%7D%5Et%20%5Ccdot%20%5Ctexttt%7Bpt2%7D%29%281%29%7D)
...
/// Estimation". In this case the function also estimates the parameters ![inline formula](https://latex.codecogs.com/png.latex?f%5Fx) and ![inline formula](https://latex.codecogs.com/png.latex?f%5Fy)
...
/// *   **CV_CALIB_FIX_FOCAL_LENGTH** Fix ![inline formula](https://latex.codecogs.com/png.latex?f%5E%7B%28j%29%7D%5Fx) and ![inline formula](https://latex.codecogs.com/png.latex?f%5E%7B%28j%29%7D%5Fy) .
/// *   **CV_CALIB_FIX_ASPECT_RATIO** Optimize ![inline formula](https://latex.codecogs.com/png.latex?f%5E%7B%28j%29%7D%5Fy) . Fix the ratio ![inline formula](https://latex.codecogs.com/png.latex?f%5E%7B%28j%29%7D%5Fx%2Ff%5E%7B%28j%29%7D%5Fy)
/// .
/// *   **CV_CALIB_SAME_FOCAL_LENGTH** Enforce ![inline formula](https://latex.codecogs.com/png.latex?f%5E%7B%280%29%7D%5Fx%3Df%5E%7B%281%29%7D%5Fx) and ![inline formula](https://latex.codecogs.com/png.latex?f%5E%7B%280%29%7D%5Fy%3Df%5E%7B%281%29%7D%5Fy) .
...
/// 
/// ![block formula](https://latex.codecogs.com/png.latex?R%5F2%3DR%2AR%5F1%0AT%5F2%3DR%2AT%5F1%20%2B%20T%2C)
/// 
...
/// homography matrices H1 and H2 . The function implements the algorithm [Hartley99](https://docs.opencv.org/3.2.0/d0/de3/citelist.html#CITEREF_Hartley99) .
/// 
/// 
/// Note:
...
/// 
/// ![image](https://docs.opencv.org/3.2.0/stereo_undistort.jpg)
/// 
/// ## C++ default parameters
...
/// sub-pixel metric from [BT98](https://docs.opencv.org/3.2.0/d0/de3/citelist.html#CITEREF_BT98) is used. Though, the color images are supported as well.
