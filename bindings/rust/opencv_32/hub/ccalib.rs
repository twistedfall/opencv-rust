...
pub trait CustomPatternTrait: core::AlgorithmTrait {
	...
	fn get_pattern_points(&mut self, original_points: &mut dyn core::ToOutputArray) -> Result<()> {
		output_array_arg!(original_points);
		unsafe { sys::cv_ccalib_CustomPattern_getPatternPoints_const__OutputArrayX(self.as_raw_CustomPattern(), original_points.as_raw__OutputArray()) }.into_result()
	}
