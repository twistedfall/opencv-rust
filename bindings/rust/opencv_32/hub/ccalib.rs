...
pub trait CustomPatternTrait: core::AlgorithmTrait {
	...
	fn get_pattern_points(&mut self, original_points: &mut dyn core::ToOutputArray) -> Result<()> {
		...
		unsafe { sys::cv_ccalib_CustomPattern_getPatternPoints_const__OutputArrayX(self.as_raw_mut_CustomPattern(), original_points.as_raw__OutputArray()) }.into_result()
	}
...
pub trait RandomPatternCornerFinderTrait {
	...
	fn load_pattern(&mut self, mut pattern_image: core::Mat) -> Result<()> {
		unsafe { sys::cv_randpattern_RandomPatternCornerFinder_loadPattern_Mat(self.as_raw_mut_RandomPatternCornerFinder(), pattern_image.as_raw_mut_Mat()) }.into_result()
	}
