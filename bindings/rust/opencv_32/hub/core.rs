...
pub trait FileNodeIteratorTrait {
	...
	fn read_raw(&mut self, fmt: &str, vec: &mut u8, max_count: size_t) -> Result<core::FileNodeIterator> {
		string_arg!(fmt);
		unsafe { sys::cv_FileNodeIterator_readRaw_const_StringX_unsigned_charX_size_t(self.as_raw_mut_FileNodeIterator(), fmt.as_ptr(), vec, max_count) }.into_result().map(|ptr| unsafe { core::FileNodeIterator::from_raw(ptr) })
	}
...
impl FileStorage {
	...
	pub fn new(source: &str, flags: i32, encoding: &str) -> Result<core::FileStorage> {
		string_arg!(source);
		string_arg!(encoding);
		unsafe { sys::cv_FileStorage_FileStorage_const_StringX_int_const_StringX(source.as_ptr(), flags, encoding.as_ptr()) }.into_result().map(|ptr| unsafe { core::FileStorage::from_raw(ptr) })
	}
...
pub trait UMatDataAutoLockTrait {
	...
	fn u(&mut self) -> core::UMatData {
		unsafe { sys::cv_UMatDataAutoLock_u(self.as_raw_mut_UMatDataAutoLock()) }.into_result().map(|ptr| unsafe { core::UMatData::from_raw(ptr) }).expect("Infallible function failed: u")
	}
	
	fn set_u(&mut self, val: &mut core::UMatData) -> () {
		unsafe { sys::cv_UMatDataAutoLock_setU_UMatDataX(self.as_raw_mut_UMatDataAutoLock(), val.as_raw_mut_UMatData()) }.into_result().expect("Infallible function failed: set_u")
	}
...
impl UMatDataAutoLock {
	pub fn new(u: &mut core::UMatData) -> Result<core::UMatDataAutoLock> {
		unsafe { sys::cv_UMatDataAutoLock_UMatDataAutoLock_UMatDataX(u.as_raw_mut_UMatData()) }.into_result().map(|ptr| unsafe { core::UMatDataAutoLock::from_raw(ptr) })
	}
...
impl KernelArg {
	...
	pub fn local() -> Result<core::KernelArg> {
		unsafe { sys::cv_ocl_KernelArg_Local() }.into_result().map(|ptr| unsafe { core::KernelArg::from_raw(ptr) })
	}
...
impl Program {
	...
	pub fn new_1(buf: &str) -> Result<core::Program> {
		string_arg!(buf);
		unsafe { sys::cv_ocl_Program_Program_const_StringX(buf.as_ptr()) }.into_result().map(|ptr| unsafe { core::Program::from_raw(ptr) })
	}
