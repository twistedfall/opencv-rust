...
pub fn estimate_rigid_transform_1(src: &dyn core::ToInputArray, dst: &dyn core::ToInputArray, full_affine: bool, ransac_max_iters: i32, ransac_good_ratio: f64, ransac_size0: i32) -> Result<core::Mat> {
	input_array_arg!(src);
	input_array_arg!(dst);
	unsafe { sys::cv_estimateRigidTransform_const__InputArrayR_const__InputArrayR_bool_int_double_int(src.as_raw__InputArray(), dst.as_raw__InputArray(), full_affine, ransac_max_iters, ransac_good_ratio, ransac_size0) }.into_result().map(|r| unsafe { core::Mat::opencv_from_extern(r) } )
}
