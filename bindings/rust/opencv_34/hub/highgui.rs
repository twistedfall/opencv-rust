...
pub fn get_window_image_rect(winname: &str) -> Result<core::Rect> {
	string_arg!(winname);
	unsafe { sys::cv_getWindowImageRect_const_StringX(winname.as_ptr()) }.into_result()
}
...
pub fn resize_window_size(winname: &str, size: core::Size) -> Result<()> {
	string_arg!(winname);
	unsafe { sys::cv_resizeWindow_const_StringX_const_SizeX(winname.as_ptr(), &size) }.into_result()
}
...
pub fn select_roi_for_window(window_name: &str, img: &dyn core::ToInputArray, show_crosshair: bool, from_center: bool) -> Result<core::Rect> {
	string_arg!(window_name);
	input_array_arg!(img);
	unsafe { sys::cv_selectROI_const_StringX_const__InputArrayX_bool_bool(window_name.as_ptr(), img.as_raw__InputArray(), show_crosshair, from_center) }.into_result()
}
