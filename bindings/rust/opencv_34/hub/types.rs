...
mod core_types {
	...
	vector_extern! { core::Vector::<bool>, *const c_void, *mut c_void,
		...
		cv_VectorOfVectorOfbool_get -> sys::Result<*mut c_void>,
		ret_map: .map(|ptr| { core::Vector::<bool>::from_raw(ptr) })
	}
	vector_non_copy_or_bool! { core::Vector::<bool> }
	
	impl<'i> core::VectorTrait<'i> for core::Vector::<core::Vector::<bool>> {
		type Arg = core::Vector::<bool>;
	
		fn with_capacity(capacity: size_t) -> Self { Self::with_capacity(capacity) }
	
		#[inline]
		fn push(&mut self, mut val: core::Vector::<bool>) {
			extern "C" { fn cv_VectorOfVectorOfbool_push(instance: *mut c_void, val: *mut c_void); }
			unsafe { cv_VectorOfVectorOfbool_push(self.as_raw_mut_VectorOfVectorOfbool(), val.as_raw_mut_VectorOfbool()) }
		}
	
		#[inline]
		fn insert(&mut self, index: size_t, mut val: core::Vector::<bool>) -> Result<()> {
			extern "C" { fn cv_VectorOfVectorOfbool_insert(instance: *mut c_void, index: size_t, val: *mut c_void); }
			core::vector_index_check(index, self.len() + 1)?;
			unsafe { cv_VectorOfVectorOfbool_insert(self.as_raw_mut_VectorOfVectorOfbool(), index, val.as_raw_mut_VectorOfbool()) }
			Ok(())
		}
	
		#[inline]
		fn set(&mut self, index: size_t, mut val: core::Vector::<bool>) -> Result<()> {
			extern "C" { fn cv_VectorOfVectorOfbool_set(instance: *mut c_void, index: size_t, val: *mut c_void); }
			core::vector_index_check(index, self.len())?;
			unsafe { cv_VectorOfVectorOfbool_set(self.as_raw_mut_VectorOfVectorOfbool(), index, val.as_raw_mut_VectorOfbool()) }
			Ok(())
		}
	
		#[inline]
		unsafe fn set_unchecked(&mut self, index: size_t, mut val: core::Vector::<bool>) {
			extern "C" { fn cv_VectorOfVectorOfbool_set(instance: *mut c_void, index: size_t, val: *mut c_void); }
			cv_VectorOfVectorOfbool_set(self.as_raw_mut_VectorOfVectorOfbool(), index, val.as_raw_mut_VectorOfbool())
		}
	}
