...
impl EncoderParams {
	#[inline] pub fn as_raw_EncoderParams(&self) -> *const c_void { self.as_raw() }
	#[inline] pub fn as_raw_mut_EncoderParams(&mut self) -> *mut c_void { self.as_raw_mut() }
}
...
pub trait RawVideoSource {
	...
	fn get_next_packet(&mut self, data: &mut &mut u8, size: &mut i32, end_of_file: &mut bool) -> Result<bool> {
		unsafe { sys::cv_cudacodec_RawVideoSource_getNextPacket_unsigned_charXX_intX_boolX(self.as_raw_mut_RawVideoSource(), data as *mut  _ as *mut  *mut  _, size, end_of_file) }.into_result()
	}
...
pub trait VideoReader {
	...
	fn next_frame(&mut self, frame: &mut dyn core::ToOutputArray) -> Result<bool> {
		output_array_arg!(frame);
		unsafe { sys::cv_cudacodec_VideoReader_nextFrame_const__OutputArrayR(self.as_raw_mut_VideoReader(), frame.as_raw__OutputArray()) }.into_result()
	}
