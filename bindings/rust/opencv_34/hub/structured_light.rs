...
//! of the codewords are multiplexed in time [pattern](https://docs.opencv.org/3.4.10/d0/de3/citelist.html#CITEREF_pattern) .
//! 
//! In this module a time-multiplexing coding strategy based on Gray encoding is implemented following the
//! (stereo) approach described in 3DUNDERWORLD algorithm [UNDERWORLD](https://docs.opencv.org/3.4.10/d0/de3/citelist.html#CITEREF_UNDERWORLD) .
...
pub const FTP: i32 = 0;
pub const PSP: i32 = 1;
/// Class implementing the Gray-code pattern, based on [UNDERWORLD](https://docs.opencv.org/3.4.10/d0/de3/citelist.html#CITEREF_UNDERWORLD).
/// 
...
/// and Fourier-assisted phase-shifting profilometry (FAPS) based on [faps](https://docs.opencv.org/3.4.10/d0/de3/citelist.html#CITEREF_faps).
/// 
...
impl dyn SinusoidalPattern + '_ {
	...
	pub fn create(parameters: types::PtrOfSinusoidalPattern_Params) -> Result<types::PtrOfSinusoidalPattern> {
		unsafe { sys::cv_structured_light_SinusoidalPattern_create_Ptr_Params_(parameters.as_raw_PtrOfSinusoidalPattern_Params()) }.into_result().map(|ptr| types::PtrOfSinusoidalPattern { ptr })
	}
