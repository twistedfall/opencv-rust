...
pub trait HDF5 {
	...
	fn hlexists(&self, label: &str) -> Result<bool> {
		string_arg!(label);
		unsafe { sys::cv_hdf_HDF5_hlexists_const_const_StringX(self.as_raw_HDF5(), label.as_ptr()) }.into_result()
	}
	...
	fn atexists(&self, atlabel: &str) -> Result<bool> {
		string_arg!(atlabel);
		unsafe { sys::cv_hdf_HDF5_atexists_const_const_StringX(self.as_raw_HDF5(), atlabel.as_ptr()) }.into_result()
	}
	...
	fn dscreate(&self, rows: i32, cols: i32, typ: i32, dslabel: &str) -> Result<()> {
		string_arg!(dslabel);
		unsafe { sys::cv_hdf_HDF5_dscreate_const_int_int_int_const_StringX(self.as_raw_HDF5(), rows, cols, typ, dslabel.as_ptr()) }.into_result()
	}
	...
	fn dscreate_1(&self, rows: i32, cols: i32, typ: i32, dslabel: &str, compresslevel: i32) -> Result<()> {
		string_arg!(dslabel);
		unsafe { sys::cv_hdf_HDF5_dscreate_const_int_int_int_const_StringX_int(self.as_raw_HDF5(), rows, cols, typ, dslabel.as_ptr(), compresslevel) }.into_result()
	}
	...
	fn dscreate_3(&self, rows: i32, cols: i32, typ: i32, dslabel: &str, compresslevel: i32, dims_chunks: &i32) -> Result<()> {
		string_arg!(dslabel);
		unsafe { sys::cv_hdf_HDF5_dscreate_const_int_int_int_const_StringX_int_const_intX(self.as_raw_HDF5(), rows, cols, typ, dslabel.as_ptr(), compresslevel, dims_chunks) }.into_result()
	}
	
	fn dscreate_4(&self, n_dims: i32, sizes: &i32, typ: i32, dslabel: &str) -> Result<()> {
		string_arg!(dslabel);
		unsafe { sys::cv_hdf_HDF5_dscreate_const_int_const_intX_int_const_StringX(self.as_raw_HDF5(), n_dims, sizes, typ, dslabel.as_ptr()) }.into_result()
	}
	
	fn dscreate_5(&self, n_dims: i32, sizes: &i32, typ: i32, dslabel: &str, compresslevel: i32) -> Result<()> {
		string_arg!(dslabel);
		unsafe { sys::cv_hdf_HDF5_dscreate_const_int_const_intX_int_const_StringX_int(self.as_raw_HDF5(), n_dims, sizes, typ, dslabel.as_ptr(), compresslevel) }.into_result()
	}
	...
	fn dscreate_7(&self, n_dims: i32, sizes: &i32, typ: i32, dslabel: &str, compresslevel: i32, dims_chunks: &i32) -> Result<()> {
		string_arg!(dslabel);
		unsafe { sys::cv_hdf_HDF5_dscreate_const_int_const_intX_int_const_StringX_int_const_intX(self.as_raw_HDF5(), n_dims, sizes, typ, dslabel.as_ptr(), compresslevel, dims_chunks) }.into_result()
	}
	...
	fn dsgettype(&self, dslabel: &str) -> Result<i32> {
		string_arg!(dslabel);
		unsafe { sys::cv_hdf_HDF5_dsgettype_const_const_StringX(self.as_raw_HDF5(), dslabel.as_ptr()) }.into_result()
	}
	
	fn dswrite(&self, array: &dyn core::ToInputArray, dslabel: &str) -> Result<()> {
		input_array_arg!(array);
		string_arg!(dslabel);
		unsafe { sys::cv_hdf_HDF5_dswrite_const_const__InputArrayX_const_StringX(self.as_raw_HDF5(), array.as_raw__InputArray(), dslabel.as_ptr()) }.into_result()
	}
	
	fn dswrite_1(&self, array: &dyn core::ToInputArray, dslabel: &str, dims_offset: &i32) -> Result<()> {
		input_array_arg!(array);
		string_arg!(dslabel);
		unsafe { sys::cv_hdf_HDF5_dswrite_const_const__InputArrayX_const_StringX_const_intX(self.as_raw_HDF5(), array.as_raw__InputArray(), dslabel.as_ptr(), dims_offset) }.into_result()
	}
	...
	fn dswrite_3(&self, array: &dyn core::ToInputArray, dslabel: &str, dims_offset: &i32, dims_counts: &i32) -> Result<()> {
		input_array_arg!(array);
		string_arg!(dslabel);
		unsafe { sys::cv_hdf_HDF5_dswrite_const_const__InputArrayX_const_StringX_const_intX_const_intX(self.as_raw_HDF5(), array.as_raw__InputArray(), dslabel.as_ptr(), dims_offset, dims_counts) }.into_result()
	}
	
	fn dsinsert(&self, array: &dyn core::ToInputArray, dslabel: &str) -> Result<()> {
		input_array_arg!(array);
		string_arg!(dslabel);
		unsafe { sys::cv_hdf_HDF5_dsinsert_const_const__InputArrayX_const_StringX(self.as_raw_HDF5(), array.as_raw__InputArray(), dslabel.as_ptr()) }.into_result()
	}
	
	fn dsinsert_1(&self, array: &dyn core::ToInputArray, dslabel: &str, dims_offset: &i32) -> Result<()> {
		input_array_arg!(array);
		string_arg!(dslabel);
		unsafe { sys::cv_hdf_HDF5_dsinsert_const_const__InputArrayX_const_StringX_const_intX(self.as_raw_HDF5(), array.as_raw__InputArray(), dslabel.as_ptr(), dims_offset) }.into_result()
	}
	...
	fn dsinsert_3(&self, array: &dyn core::ToInputArray, dslabel: &str, dims_offset: &i32, dims_counts: &i32) -> Result<()> {
		input_array_arg!(array);
		string_arg!(dslabel);
		unsafe { sys::cv_hdf_HDF5_dsinsert_const_const__InputArrayX_const_StringX_const_intX_const_intX(self.as_raw_HDF5(), array.as_raw__InputArray(), dslabel.as_ptr(), dims_offset, dims_counts) }.into_result()
	}
	
	fn dsread(&self, array: &mut dyn core::ToOutputArray, dslabel: &str) -> Result<()> {
		output_array_arg!(array);
		string_arg!(dslabel);
		unsafe { sys::cv_hdf_HDF5_dsread_const_const__OutputArrayX_const_StringX(self.as_raw_HDF5(), array.as_raw__OutputArray(), dslabel.as_ptr()) }.into_result()
	}
	
	fn dsread_1(&self, array: &mut dyn core::ToOutputArray, dslabel: &str, dims_offset: &i32) -> Result<()> {
		output_array_arg!(array);
		string_arg!(dslabel);
		unsafe { sys::cv_hdf_HDF5_dsread_const_const__OutputArrayX_const_StringX_const_intX(self.as_raw_HDF5(), array.as_raw__OutputArray(), dslabel.as_ptr(), dims_offset) }.into_result()
	}
	...
	fn dsread_3(&self, array: &mut dyn core::ToOutputArray, dslabel: &str, dims_offset: &i32, dims_counts: &i32) -> Result<()> {
		output_array_arg!(array);
		string_arg!(dslabel);
		unsafe { sys::cv_hdf_HDF5_dsread_const_const__OutputArrayX_const_StringX_const_intX_const_intX(self.as_raw_HDF5(), array.as_raw__OutputArray(), dslabel.as_ptr(), dims_offset, dims_counts) }.into_result()
	}
	...
	fn kpgetsize(&self, kplabel: &str, dims_flag: i32) -> Result<i32> {
		string_arg!(kplabel);
		unsafe { sys::cv_hdf_HDF5_kpgetsize_const_const_StringX_int(self.as_raw_HDF5(), kplabel.as_ptr(), dims_flag) }.into_result()
	}
	...
	fn kpcreate(&self, size: i32, kplabel: &str, compresslevel: i32, chunks: i32) -> Result<()> {
		string_arg!(kplabel);
		unsafe { sys::cv_hdf_HDF5_kpcreate_const_int_const_StringX_int_int(self.as_raw_HDF5(), size, kplabel.as_ptr(), compresslevel, chunks) }.into_result()
	}
