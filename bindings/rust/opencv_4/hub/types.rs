...
mod calib3d_types {
	...
	impl PtrOfLMSolver {
		pub fn as_raw_PtrOfLMSolver(&self) -> *mut c_void { self.ptr }
		...
		/// Get raw pointer to the inner object
		pub fn get_inner_ptr(&self) -> *mut c_void {
			extern "C" { fn cv_PtrOfLMSolver_get_inner_ptr(instance: *mut c_void) -> *mut c_void; }
			unsafe { cv_PtrOfLMSolver_get_inner_ptr(self.as_raw_PtrOfLMSolver()) }
		}
	}
	
	impl Drop for PtrOfLMSolver {
		fn drop(&mut self) {
			extern "C" { fn cv_PtrOfLMSolver_delete(instance: *mut c_void); }
			unsafe { cv_PtrOfLMSolver_delete(self.as_raw_PtrOfLMSolver()) };
		}
	}
	...
	pub struct PtrOfLMSolver_Callback {
		pub(crate) ptr: *mut c_void
	}
	
	impl PtrOfLMSolver_Callback {
		pub fn as_raw_PtrOfLMSolver_Callback(&self) -> *mut c_void { self.ptr }
		...
		/// Get raw pointer to the inner object
		pub fn get_inner_ptr(&self) -> *mut c_void {
			extern "C" { fn cv_PtrOfLMSolver_Callback_get_inner_ptr(instance: *mut c_void) -> *mut c_void; }
			unsafe { cv_PtrOfLMSolver_Callback_get_inner_ptr(self.as_raw_PtrOfLMSolver_Callback()) }
		}
	}
	
	impl Drop for PtrOfLMSolver_Callback {
		fn drop(&mut self) {
			extern "C" { fn cv_PtrOfLMSolver_Callback_delete(instance: *mut c_void); }
			unsafe { cv_PtrOfLMSolver_Callback_delete(self.as_raw_PtrOfLMSolver_Callback()) };
		}
	}
	
	unsafe impl Send for PtrOfLMSolver_Callback {}
...
mod core_types {
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfRect2d {
		...
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			...
			cv_VectorOfRect2d_get_unchecked(self.as_raw_VectorOfRect2d(), index)
				.into_result()
				.expect("Infallible function failed: VectorOfRect2d::get_unchecked")
		}
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfRotatedRect {
		...
		#[inline]
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			extern "C" { fn cv_VectorOfRotatedRect_get_unchecked(instance: *mut c_void, index: size_t) -> sys::Result<*mut c_void>; }
			cv_VectorOfRotatedRect_get_unchecked(self.as_raw_VectorOfRotatedRect(), index)
				.into_result()
				.map(|ptr| core::RotatedRect { ptr })
				.expect("Infallible function failed: VectorOfRotatedRect::get_unchecked")
		}
	...
	pub struct VectorOfScalar {
		pub(crate) ptr: *mut c_void
	}
	
	impl VectorOfScalar {
		pub fn as_raw_VectorOfScalar(&self) -> *mut c_void { self.ptr }
		...
		pub fn to_slice(&self) -> &[core::Scalar] {
			extern "C" { fn cv_VectorOfScalar_data(instance: *mut c_void) -> *const core::Scalar; }
			unsafe {
				let data = cv_VectorOfScalar_data(self.as_raw_VectorOfScalar());
				::std::slice::from_raw_parts(data, crate::templ::Vector::len(self))
			}
		}
	}
	
	impl Drop for VectorOfScalar {
		#[inline]
		fn drop(&mut self) {
			extern "C" { fn cv_VectorOfScalar_delete(instance: *mut c_void); }
			unsafe { cv_VectorOfScalar_delete(self.as_raw_VectorOfScalar()) };
		}
	}
	
	impl IntoIterator for VectorOfScalar {
		type Item = core::Scalar;
		type IntoIter = crate::templ::VectorIterator<Self>;
	...
	impl<'i> IntoIterator for &'i VectorOfScalar {
		type Item = core::Scalar;
		type IntoIter = crate::templ::VectorRefIterator<'i, VectorOfScalar>;
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfScalar {
		type Storage = core::Scalar;
		type Arg = core::Scalar;
	
		#[inline]
		fn new() -> Self {
			extern "C" { fn cv_VectorOfScalar_new() -> *mut c_void; }
			Self { ptr: unsafe { cv_VectorOfScalar_new() } }
		}
	
		#[inline]
		fn len(&self) -> size_t {
			extern "C" { fn cv_VectorOfScalar_len(instance: *mut c_void) -> size_t; }
			unsafe { cv_VectorOfScalar_len(self.as_raw_VectorOfScalar()) }
		}
	
		#[inline]
		fn is_empty(&self) -> bool {
			extern "C" { fn cv_VectorOfScalar_is_empty(instance: *mut c_void) -> bool; }
			unsafe { cv_VectorOfScalar_is_empty(self.as_raw_VectorOfScalar()) }
		}
	
		#[inline]
		fn capacity(&self) -> size_t {
			extern "C" { fn cv_VectorOfScalar_capacity(instance: *mut c_void) -> size_t; }
			unsafe { cv_VectorOfScalar_capacity(self.as_raw_VectorOfScalar()) }
		}
	
		#[inline]
		fn shrink_to_fit(&mut self) {
			extern "C" { fn cv_VectorOfScalar_shrink_to_fit(instance: *mut c_void); }
			unsafe { cv_VectorOfScalar_shrink_to_fit(self.as_raw_VectorOfScalar()) }
		}
	
		#[inline]
		fn reserve(&mut self, additional: size_t) {
			extern "C" { fn cv_VectorOfScalar_reserve(instance: *mut c_void, additional: size_t); }
			unsafe { cv_VectorOfScalar_reserve(self.as_raw_VectorOfScalar(), additional) }
		}
	
		#[inline]
		fn remove(&mut self, index: size_t) -> Result<()> {
			crate::templ::vector_index_check(index, self.len())?;
			extern "C" { fn cv_VectorOfScalar_remove(instance: *mut c_void, index: size_t); }
			unsafe { cv_VectorOfScalar_remove(self.as_raw_VectorOfScalar(), index) };
			Ok(())
		}
		...
		fn swap(&mut self, index1: size_t, index2: size_t) -> Result<()> {
			...
			crate::templ::vector_index_check(index2, len)?;
			if index1 != index2 {
				extern "C" { fn cv_VectorOfScalar_swap(instance: *mut c_void, index1: size_t, index2: size_t); }
				unsafe { cv_VectorOfScalar_swap(self.as_raw_VectorOfScalar(), index1, index2) };
			}
			Ok(())
		}
		...
		#[inline]
		fn clear(&mut self) {
			extern "C" { fn cv_VectorOfScalar_clear(instance: *mut c_void); }
			unsafe { cv_VectorOfScalar_clear(self.as_raw_VectorOfScalar()) }
		}
	
		#[inline]
		fn push(&mut self, val: Self::Arg) {
			extern "C" { fn cv_VectorOfScalar_push(instance: *mut c_void, val: *const core::Scalar); }
			unsafe { cv_VectorOfScalar_push(self.as_raw_VectorOfScalar(), &val) }
		}
		...
		fn insert(&mut self, index: size_t, val: Self::Arg) -> Result<()> {
			...
			crate::templ::vector_index_check(index, self.len() + 1)?;
			unsafe { cv_VectorOfScalar_insert(self.as_raw_VectorOfScalar(), index, &val) }
			Ok(())
		}
	
		#[inline]
		fn get(&self, index: size_t) -> Result<Self::Storage> {
			extern "C" { fn cv_VectorOfScalar_get(instance: *mut c_void, index: size_t) -> sys::Result<core::Scalar>; }
			unsafe { cv_VectorOfScalar_get(self.as_raw_VectorOfScalar(), index) }
				.into_result()
		}
	
		#[inline]
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			extern "C" { fn cv_VectorOfScalar_get_unchecked(instance: *mut c_void, index: size_t) -> sys::Result<core::Scalar>; }
			cv_VectorOfScalar_get_unchecked(self.as_raw_VectorOfScalar(), index)
				.into_result()
				.expect("Infallible function failed: VectorOfScalar::get_unchecked")
		}
	
		#[inline]
		fn set(&mut self, index: size_t, val: Self::Arg) -> Result<()> {
			extern "C" { fn cv_VectorOfScalar_set(instance: *mut c_void, index: size_t, val: *const core::Scalar) -> sys::Result_void; }
			unsafe { cv_VectorOfScalar_set(self.as_raw_VectorOfScalar(), index, &val) }
				.into_result()
		}
	
		#[inline]
		unsafe fn set_unchecked(&mut self, index: size_t, val: Self::Arg) {
			extern "C" { fn cv_VectorOfScalar_set_unchecked(instance: *mut c_void, index: size_t, val: *const core::Scalar); }
			cv_VectorOfScalar_set_unchecked(self.as_raw_VectorOfScalar(), index, &val)
		}
	...
	unsafe impl Send for VectorOfScalar {}
	
	impl core::ToInputArray for VectorOfScalar {
		#[inline]
		fn input_array(&self) -> Result<core::_InputArray> {
			extern "C" { fn cv_VectorOfScalar_input_array(instance: *mut c_void) -> sys::Result<*mut c_void>; }
	...
	impl core::ToOutputArray for VectorOfScalar {
		...
		fn output_array(&mut self) -> Result<core::_OutputArray> {
			...
			unsafe { cv_VectorOfScalar_output_array(self.as_raw_VectorOfScalar()) }
				.into_result()
				.map(|ptr| core::_OutputArray { ptr })
		}
	}
	...
	impl core::ToInputOutputArray for VectorOfScalar {
		...
		fn input_output_array(&mut self) -> Result<core::_InputOutputArray> {
			...
			unsafe { cv_VectorOfScalar_input_output_array(self.as_raw_VectorOfScalar()) }
				.into_result()
				.map(|ptr| core::_InputOutputArray { ptr })
		}
	}
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfVec3i {
		...
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			...
			cv_VectorOfVec3i_get_unchecked(self.as_raw_VectorOfVec3i(), index)
				.into_result()
				.expect("Infallible function failed: VectorOfVec3i::get_unchecked")
		}
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfVectorOfMat {
		...
		#[inline]
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			extern "C" { fn cv_VectorOfVectorOfMat_get_unchecked(instance: *mut c_void, index: size_t) -> sys::Result<*mut c_void>; }
			cv_VectorOfVectorOfMat_get_unchecked(self.as_raw_VectorOfVectorOfMat(), index)
				.into_result()
				.map(|ptr| types::VectorOfMat { ptr })
				.expect("Infallible function failed: VectorOfVectorOfMat::get_unchecked")
		}
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfVectorOfRange {
		...
		#[inline]
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			extern "C" { fn cv_VectorOfVectorOfRange_get_unchecked(instance: *mut c_void, index: size_t) -> sys::Result<*mut c_void>; }
			cv_VectorOfVectorOfRange_get_unchecked(self.as_raw_VectorOfVectorOfRange(), index)
				.into_result()
				.map(|ptr| types::VectorOfRange { ptr })
				.expect("Infallible function failed: VectorOfVectorOfRange::get_unchecked")
		}
...
mod dnn_types {
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfMatShape {
		...
		#[inline]
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			extern "C" { fn cv_VectorOfMatShape_get_unchecked(instance: *mut c_void, index: size_t) -> sys::Result<*mut c_void>; }
			cv_VectorOfMatShape_get_unchecked(self.as_raw_VectorOfMatShape(), index)
				.into_result()
				.map(|ptr| types::VectorOfi32 { ptr })
				.expect("Infallible function failed: VectorOfMatShape::get_unchecked")
		}
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfPtrOfBackendNode {
		...
		#[inline]
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			extern "C" { fn cv_VectorOfPtrOfBackendNode_get_unchecked(instance: *mut c_void, index: size_t) -> sys::Result<*mut c_void>; }
			cv_VectorOfPtrOfBackendNode_get_unchecked(self.as_raw_VectorOfPtrOfBackendNode(), index)
				.into_result()
				.map(|ptr| types::PtrOfBackendNode { ptr })
				.expect("Infallible function failed: VectorOfPtrOfBackendNode::get_unchecked")
		}
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfPtrOfBackendWrapper {
		...
		#[inline]
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			extern "C" { fn cv_VectorOfPtrOfBackendWrapper_get_unchecked(instance: *mut c_void, index: size_t) -> sys::Result<*mut c_void>; }
			cv_VectorOfPtrOfBackendWrapper_get_unchecked(self.as_raw_VectorOfPtrOfBackendWrapper(), index)
				.into_result()
				.map(|ptr| types::PtrOfBackendWrapper { ptr })
				.expect("Infallible function failed: VectorOfPtrOfBackendWrapper::get_unchecked")
		}
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfPtrOfLayer {
		...
		#[inline]
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			extern "C" { fn cv_VectorOfPtrOfLayer_get_unchecked(instance: *mut c_void, index: size_t) -> sys::Result<*mut c_void>; }
			cv_VectorOfPtrOfLayer_get_unchecked(self.as_raw_VectorOfPtrOfLayer(), index)
				.into_result()
				.map(|ptr| types::PtrOfLayer { ptr })
				.expect("Infallible function failed: VectorOfPtrOfLayer::get_unchecked")
		}
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfTarget {
		...
		#[inline]
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			extern "C" { fn cv_VectorOfTarget_get_unchecked(instance: *mut c_void, index: size_t) -> sys::Result<crate::dnn::Target>; }
			cv_VectorOfTarget_get_unchecked(self.as_raw_VectorOfTarget(), index)
				.into_result()
				.expect("Infallible function failed: VectorOfTarget::get_unchecked")
		}
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfVectorOfMatShape {
		...
		#[inline]
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			extern "C" { fn cv_VectorOfVectorOfMatShape_get_unchecked(instance: *mut c_void, index: size_t) -> sys::Result<*mut c_void>; }
			cv_VectorOfVectorOfMatShape_get_unchecked(self.as_raw_VectorOfVectorOfMatShape(), index)
				.into_result()
				.map(|ptr| types::VectorOfMatShape { ptr })
				.expect("Infallible function failed: VectorOfVectorOfMatShape::get_unchecked")
		}
...
mod face_types {
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfFacemarkAAM_Config {
		...
		#[inline]
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			extern "C" { fn cv_VectorOfFacemarkAAM_Config_get_unchecked(instance: *mut c_void, index: size_t) -> sys::Result<*mut c_void>; }
			cv_VectorOfFacemarkAAM_Config_get_unchecked(self.as_raw_VectorOfFacemarkAAM_Config(), index)
				.into_result()
				.map(|ptr| crate::face::FacemarkAAM_Config { ptr })
				.expect("Infallible function failed: VectorOfFacemarkAAM_Config::get_unchecked")
		}
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfFacemarkAAM_Model_Texture {
		...
		#[inline]
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			extern "C" { fn cv_VectorOfFacemarkAAM_Model_Texture_get_unchecked(instance: *mut c_void, index: size_t) -> sys::Result<*mut c_void>; }
			cv_VectorOfFacemarkAAM_Model_Texture_get_unchecked(self.as_raw_VectorOfFacemarkAAM_Model_Texture(), index)
				.into_result()
				.map(|ptr| crate::face::FacemarkAAM_Model_Texture { ptr })
				.expect("Infallible function failed: VectorOfFacemarkAAM_Model_Texture::get_unchecked")
		}
...
mod features2d_types {
	...
	impl PtrOfFeature2D {
		...
		/// Get raw pointer to the inner object
		pub fn get_inner_ptr(&self) -> *mut c_void {
			extern "C" { fn cv_PtrOfFeature2D_get_inner_ptr(instance: *mut c_void) -> *mut c_void; }
			unsafe { cv_PtrOfFeature2D_get_inner_ptr(self.as_raw_PtrOfFeature2D()) }
		}
	}
	
	impl Drop for PtrOfFeature2D {
		fn drop(&mut self) {
			extern "C" { fn cv_PtrOfFeature2D_delete(instance: *mut c_void); }
			unsafe { cv_PtrOfFeature2D_delete(self.as_raw_PtrOfFeature2D()) };
		}
	}
...
mod flann_types {
	...
	pub struct VectorOfFlannIndexType {
		pub(crate) ptr: *mut c_void
	}
	
	impl VectorOfFlannIndexType {
		pub fn as_raw_VectorOfFlannIndexType(&self) -> *mut c_void { self.ptr }
		...
		pub fn to_slice(&self) -> &[crate::flann::FlannIndexType] {
			extern "C" { fn cv_VectorOfFlannIndexType_data(instance: *mut c_void) -> *const crate::flann::FlannIndexType; }
			unsafe {
				let data = cv_VectorOfFlannIndexType_data(self.as_raw_VectorOfFlannIndexType());
				::std::slice::from_raw_parts(data, crate::templ::Vector::len(self))
			}
		}
	}
	
	impl Drop for VectorOfFlannIndexType {
		#[inline]
		fn drop(&mut self) {
			extern "C" { fn cv_VectorOfFlannIndexType_delete(instance: *mut c_void); }
			unsafe { cv_VectorOfFlannIndexType_delete(self.as_raw_VectorOfFlannIndexType()) };
		}
	}
	
	impl IntoIterator for VectorOfFlannIndexType {
		type Item = crate::flann::FlannIndexType;
		type IntoIter = crate::templ::VectorIterator<Self>;
	...
	impl<'i> IntoIterator for &'i VectorOfFlannIndexType {
		type Item = crate::flann::FlannIndexType;
		type IntoIter = crate::templ::VectorRefIterator<'i, VectorOfFlannIndexType>;
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfFlannIndexType {
		type Storage = crate::flann::FlannIndexType;
		type Arg = crate::flann::FlannIndexType;
	
		#[inline]
		fn new() -> Self {
			extern "C" { fn cv_VectorOfFlannIndexType_new() -> *mut c_void; }
			Self { ptr: unsafe { cv_VectorOfFlannIndexType_new() } }
		}
	
		#[inline]
		fn len(&self) -> size_t {
			extern "C" { fn cv_VectorOfFlannIndexType_len(instance: *mut c_void) -> size_t; }
			unsafe { cv_VectorOfFlannIndexType_len(self.as_raw_VectorOfFlannIndexType()) }
		}
	
		#[inline]
		fn is_empty(&self) -> bool {
			extern "C" { fn cv_VectorOfFlannIndexType_is_empty(instance: *mut c_void) -> bool; }
			unsafe { cv_VectorOfFlannIndexType_is_empty(self.as_raw_VectorOfFlannIndexType()) }
		}
	
		#[inline]
		fn capacity(&self) -> size_t {
			extern "C" { fn cv_VectorOfFlannIndexType_capacity(instance: *mut c_void) -> size_t; }
			unsafe { cv_VectorOfFlannIndexType_capacity(self.as_raw_VectorOfFlannIndexType()) }
		}
	
		#[inline]
		fn shrink_to_fit(&mut self) {
			extern "C" { fn cv_VectorOfFlannIndexType_shrink_to_fit(instance: *mut c_void); }
			unsafe { cv_VectorOfFlannIndexType_shrink_to_fit(self.as_raw_VectorOfFlannIndexType()) }
		}
	
		#[inline]
		fn reserve(&mut self, additional: size_t) {
			extern "C" { fn cv_VectorOfFlannIndexType_reserve(instance: *mut c_void, additional: size_t); }
			unsafe { cv_VectorOfFlannIndexType_reserve(self.as_raw_VectorOfFlannIndexType(), additional) }
		}
	
		#[inline]
		fn remove(&mut self, index: size_t) -> Result<()> {
			crate::templ::vector_index_check(index, self.len())?;
			extern "C" { fn cv_VectorOfFlannIndexType_remove(instance: *mut c_void, index: size_t); }
			unsafe { cv_VectorOfFlannIndexType_remove(self.as_raw_VectorOfFlannIndexType(), index) };
			Ok(())
		}
		...
		fn swap(&mut self, index1: size_t, index2: size_t) -> Result<()> {
			...
			crate::templ::vector_index_check(index2, len)?;
			if index1 != index2 {
				extern "C" { fn cv_VectorOfFlannIndexType_swap(instance: *mut c_void, index1: size_t, index2: size_t); }
				unsafe { cv_VectorOfFlannIndexType_swap(self.as_raw_VectorOfFlannIndexType(), index1, index2) };
			}
			Ok(())
		}
		...
		#[inline]
		fn clear(&mut self) {
			extern "C" { fn cv_VectorOfFlannIndexType_clear(instance: *mut c_void); }
			unsafe { cv_VectorOfFlannIndexType_clear(self.as_raw_VectorOfFlannIndexType()) }
		}
	
		#[inline]
		fn push(&mut self, val: Self::Arg) {
			extern "C" { fn cv_VectorOfFlannIndexType_push(instance: *mut c_void, val: crate::flann::FlannIndexType); }
			unsafe { cv_VectorOfFlannIndexType_push(self.as_raw_VectorOfFlannIndexType(), val) }
		}
		...
		fn insert(&mut self, index: size_t, val: Self::Arg) -> Result<()> {
			...
			crate::templ::vector_index_check(index, self.len() + 1)?;
			unsafe { cv_VectorOfFlannIndexType_insert(self.as_raw_VectorOfFlannIndexType(), index, val) }
			Ok(())
		}
	
		#[inline]
		fn get(&self, index: size_t) -> Result<Self::Storage> {
			extern "C" { fn cv_VectorOfFlannIndexType_get(instance: *mut c_void, index: size_t) -> sys::Result<crate::flann::FlannIndexType>; }
			unsafe { cv_VectorOfFlannIndexType_get(self.as_raw_VectorOfFlannIndexType(), index) }
				.into_result()
		}
	
		#[inline]
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			extern "C" { fn cv_VectorOfFlannIndexType_get_unchecked(instance: *mut c_void, index: size_t) -> sys::Result<crate::flann::FlannIndexType>; }
			cv_VectorOfFlannIndexType_get_unchecked(self.as_raw_VectorOfFlannIndexType(), index)
				.into_result()
				.expect("Infallible function failed: VectorOfFlannIndexType::get_unchecked")
		}
	
		#[inline]
		fn set(&mut self, index: size_t, val: Self::Arg) -> Result<()> {
			extern "C" { fn cv_VectorOfFlannIndexType_set(instance: *mut c_void, index: size_t, val: crate::flann::FlannIndexType) -> sys::Result_void; }
			unsafe { cv_VectorOfFlannIndexType_set(self.as_raw_VectorOfFlannIndexType(), index, val) }
				.into_result()
		}
	
		#[inline]
		unsafe fn set_unchecked(&mut self, index: size_t, val: Self::Arg) {
			extern "C" { fn cv_VectorOfFlannIndexType_set_unchecked(instance: *mut c_void, index: size_t, val: crate::flann::FlannIndexType); }
			cv_VectorOfFlannIndexType_set_unchecked(self.as_raw_VectorOfFlannIndexType(), index, val)
		}
	...
	unsafe impl Send for VectorOfFlannIndexType {}
...
mod stitching_types {
	...
	pub struct PtrOfDetail_BestOf2NearestMatcher {
		pub(crate) ptr: *mut c_void
	}
	
	impl PtrOfDetail_BestOf2NearestMatcher {
		pub fn as_raw_PtrOfDetail_BestOf2NearestMatcher(&self) -> *mut c_void { self.ptr }
		...
		/// Get raw pointer to the inner object
		pub fn get_inner_ptr(&self) -> *mut c_void {
			extern "C" { fn cv_PtrOfDetail_BestOf2NearestMatcher_get_inner_ptr(instance: *mut c_void) -> *mut c_void; }
			unsafe { cv_PtrOfDetail_BestOf2NearestMatcher_get_inner_ptr(self.as_raw_PtrOfDetail_BestOf2NearestMatcher()) }
		}
	}
	
	impl Drop for PtrOfDetail_BestOf2NearestMatcher {
		fn drop(&mut self) {
			extern "C" { fn cv_PtrOfDetail_BestOf2NearestMatcher_delete(instance: *mut c_void); }
			unsafe { cv_PtrOfDetail_BestOf2NearestMatcher_delete(self.as_raw_PtrOfDetail_BestOf2NearestMatcher()) };
		}
	}
	
	unsafe impl Send for PtrOfDetail_BestOf2NearestMatcher {}
	...
	pub struct PtrOfDetail_Estimator {
		pub(crate) ptr: *mut c_void
	}
	
	impl PtrOfDetail_Estimator {
		pub fn as_raw_PtrOfDetail_Estimator(&self) -> *mut c_void { self.ptr }
		...
		/// Get raw pointer to the inner object
		pub fn get_inner_ptr(&self) -> *mut c_void {
			extern "C" { fn cv_PtrOfDetail_Estimator_get_inner_ptr(instance: *mut c_void) -> *mut c_void; }
			unsafe { cv_PtrOfDetail_Estimator_get_inner_ptr(self.as_raw_PtrOfDetail_Estimator()) }
		}
	}
	
	impl Drop for PtrOfDetail_Estimator {
		fn drop(&mut self) {
			extern "C" { fn cv_PtrOfDetail_Estimator_delete(instance: *mut c_void); }
			unsafe { cv_PtrOfDetail_Estimator_delete(self.as_raw_PtrOfDetail_Estimator()) };
		}
	}
	
	unsafe impl Send for PtrOfDetail_Estimator {}
...
mod video_types {
	...
	impl PtrOfDISOpticalFlow {
		pub fn as_raw_PtrOfDISOpticalFlow(&self) -> *mut c_void { self.ptr }
		...
		/// Get raw pointer to the inner object
		pub fn get_inner_ptr(&self) -> *mut c_void {
			extern "C" { fn cv_PtrOfDISOpticalFlow_get_inner_ptr(instance: *mut c_void) -> *mut c_void; }
			unsafe { cv_PtrOfDISOpticalFlow_get_inner_ptr(self.as_raw_PtrOfDISOpticalFlow()) }
		}
	}
	
	impl Drop for PtrOfDISOpticalFlow {
		fn drop(&mut self) {
			extern "C" { fn cv_PtrOfDISOpticalFlow_delete(instance: *mut c_void); }
			unsafe { cv_PtrOfDISOpticalFlow_delete(self.as_raw_PtrOfDISOpticalFlow()) };
		}
	}
	...
	impl PtrOfVariationalRefinement {
		pub fn as_raw_PtrOfVariationalRefinement(&self) -> *mut c_void { self.ptr }
		...
		/// Get raw pointer to the inner object
		pub fn get_inner_ptr(&self) -> *mut c_void {
			extern "C" { fn cv_PtrOfVariationalRefinement_get_inner_ptr(instance: *mut c_void) -> *mut c_void; }
			unsafe { cv_PtrOfVariationalRefinement_get_inner_ptr(self.as_raw_PtrOfVariationalRefinement()) }
		}
	}
	
	impl Drop for PtrOfVariationalRefinement {
		fn drop(&mut self) {
			extern "C" { fn cv_PtrOfVariationalRefinement_delete(instance: *mut c_void); }
			unsafe { cv_PtrOfVariationalRefinement_delete(self.as_raw_PtrOfVariationalRefinement()) };
		}
	}
...
mod videoio_types {
	...
	impl VectorOfVideoCapture {
		pub fn as_raw_VectorOfVideoCapture(&self) -> *mut c_void { self.ptr }
	...
	impl Drop for VectorOfVideoCapture {
		#[inline]
		fn drop(&mut self) {
			extern "C" { fn cv_VectorOfVideoCapture_delete(instance: *mut c_void); }
			unsafe { cv_VectorOfVideoCapture_delete(self.as_raw_VectorOfVideoCapture()) };
		}
	}
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfVideoCapture {
		...
		#[inline]
		fn new() -> Self {
			extern "C" { fn cv_VectorOfVideoCapture_new() -> *mut c_void; }
			Self { ptr: unsafe { cv_VectorOfVideoCapture_new() } }
		}
	
		#[inline]
		fn len(&self) -> size_t {
			extern "C" { fn cv_VectorOfVideoCapture_len(instance: *mut c_void) -> size_t; }
			unsafe { cv_VectorOfVideoCapture_len(self.as_raw_VectorOfVideoCapture()) }
		}
	
		#[inline]
		fn is_empty(&self) -> bool {
			extern "C" { fn cv_VectorOfVideoCapture_is_empty(instance: *mut c_void) -> bool; }
			unsafe { cv_VectorOfVideoCapture_is_empty(self.as_raw_VectorOfVideoCapture()) }
		}
	
		#[inline]
		fn capacity(&self) -> size_t {
			extern "C" { fn cv_VectorOfVideoCapture_capacity(instance: *mut c_void) -> size_t; }
			unsafe { cv_VectorOfVideoCapture_capacity(self.as_raw_VectorOfVideoCapture()) }
		}
	
		#[inline]
		fn shrink_to_fit(&mut self) {
			extern "C" { fn cv_VectorOfVideoCapture_shrink_to_fit(instance: *mut c_void); }
			unsafe { cv_VectorOfVideoCapture_shrink_to_fit(self.as_raw_VectorOfVideoCapture()) }
		}
	
		#[inline]
		fn reserve(&mut self, additional: size_t) {
			extern "C" { fn cv_VectorOfVideoCapture_reserve(instance: *mut c_void, additional: size_t); }
			unsafe { cv_VectorOfVideoCapture_reserve(self.as_raw_VectorOfVideoCapture(), additional) }
		}
	
		#[inline]
		fn remove(&mut self, index: size_t) -> Result<()> {
			crate::templ::vector_index_check(index, self.len())?;
			extern "C" { fn cv_VectorOfVideoCapture_remove(instance: *mut c_void, index: size_t); }
			unsafe { cv_VectorOfVideoCapture_remove(self.as_raw_VectorOfVideoCapture(), index) };
			Ok(())
		}
		...
		fn swap(&mut self, index1: size_t, index2: size_t) -> Result<()> {
			...
			crate::templ::vector_index_check(index2, len)?;
			if index1 != index2 {
				extern "C" { fn cv_VectorOfVideoCapture_swap(instance: *mut c_void, index1: size_t, index2: size_t); }
				unsafe { cv_VectorOfVideoCapture_swap(self.as_raw_VectorOfVideoCapture(), index1, index2) };
			}
			Ok(())
		}
		...
		#[inline]
		fn clear(&mut self) {
			extern "C" { fn cv_VectorOfVideoCapture_clear(instance: *mut c_void); }
			unsafe { cv_VectorOfVideoCapture_clear(self.as_raw_VectorOfVideoCapture()) }
		}
	
		#[inline]
		fn push(&mut self, val: Self::Arg) {
			extern "C" { fn cv_VectorOfVideoCapture_push(instance: *mut c_void, val: *mut c_void); }
			unsafe { cv_VectorOfVideoCapture_push(self.as_raw_VectorOfVideoCapture(), val.as_raw_VideoCapture()) }
		}
		...
		fn insert(&mut self, index: size_t, val: Self::Arg) -> Result<()> {
			...
			crate::templ::vector_index_check(index, self.len() + 1)?;
			unsafe { cv_VectorOfVideoCapture_insert(self.as_raw_VectorOfVideoCapture(), index, val.as_raw_VideoCapture()) }
			Ok(())
		}
	
		#[inline]
		fn get(&self, index: size_t) -> Result<Self::Storage> {
			extern "C" { fn cv_VectorOfVideoCapture_get(instance: *mut c_void, index: size_t) -> sys::Result<*mut c_void>; }
			unsafe { cv_VectorOfVideoCapture_get(self.as_raw_VectorOfVideoCapture(), index) }
				.into_result()
				.map(|ptr| crate::videoio::VideoCapture { ptr })
		}
	
		#[inline]
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			extern "C" { fn cv_VectorOfVideoCapture_get_unchecked(instance: *mut c_void, index: size_t) -> sys::Result<*mut c_void>; }
			cv_VectorOfVideoCapture_get_unchecked(self.as_raw_VectorOfVideoCapture(), index)
				.into_result()
				.map(|ptr| crate::videoio::VideoCapture { ptr })
				.expect("Infallible function failed: VectorOfVideoCapture::get_unchecked")
		}
	
		#[inline]
		fn set(&mut self, index: size_t, val: Self::Arg) -> Result<()> {
			extern "C" { fn cv_VectorOfVideoCapture_set(instance: *mut c_void, index: size_t, val: *mut c_void) -> sys::Result_void; }
			unsafe { cv_VectorOfVideoCapture_set(self.as_raw_VectorOfVideoCapture(), index, val.as_raw_VideoCapture()) }
				.into_result()
		}
	
		#[inline]
		unsafe fn set_unchecked(&mut self, index: size_t, val: Self::Arg) {
			extern "C" { fn cv_VectorOfVideoCapture_set_unchecked(instance: *mut c_void, index: size_t, val: *mut c_void); }
			cv_VectorOfVideoCapture_set_unchecked(self.as_raw_VectorOfVideoCapture(), index, val.as_raw_VideoCapture())
		}
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfVideoCaptureAPIs {
		...
		#[inline]
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			extern "C" { fn cv_VectorOfVideoCaptureAPIs_get_unchecked(instance: *mut c_void, index: size_t) -> sys::Result<crate::videoio::VideoCaptureAPIs>; }
			cv_VectorOfVideoCaptureAPIs_get_unchecked(self.as_raw_VectorOfVideoCaptureAPIs(), index)
				.into_result()
				.expect("Infallible function failed: VectorOfVideoCaptureAPIs::get_unchecked")
		}
...
mod xfeatures2d_types {
	...
	impl<'i> crate::templ::Vector<'i> for VectorOfElliptic_KeyPoint {
		...
		#[inline]
		unsafe fn get_unchecked(&self, index: size_t) -> Self::Storage {
			extern "C" { fn cv_VectorOfElliptic_KeyPoint_get_unchecked(instance: *mut c_void, index: size_t) -> sys::Result<*mut c_void>; }
			cv_VectorOfElliptic_KeyPoint_get_unchecked(self.as_raw_VectorOfElliptic_KeyPoint(), index)
				.into_result()
				.map(|ptr| crate::xfeatures2d::Elliptic_KeyPoint { ptr })
				.expect("Infallible function failed: VectorOfElliptic_KeyPoint::get_unchecked")
		}
