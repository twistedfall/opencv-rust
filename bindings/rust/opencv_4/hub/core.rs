...
pub enum AccessFlag {
	ACCESS_READ = 16777216 as isize,
	ACCESS_WRITE = 33554432 as isize,
	ACCESS_RW = 50331648 as isize,
	// ACCESS_MASK = 50331648 as isize, // duplicate discriminant
	ACCESS_FAST = 67108864 as isize,
}
...
pub enum FileStorage_State {
	UNDEFINED = 0 as isize,
	VALUE_EXPECTED = 1 as isize,
	NAME_EXPECTED = 2 as isize,
	INSIDE_MAP = 4 as isize,
}
...
pub enum Formatter_FormatType {
	FMT_DEFAULT = 0 as isize,
	FMT_MATLAB = 1 as isize,
	FMT_CSV = 2 as isize,
	FMT_PYTHON = 3 as isize,
	FMT_NUMPY = 4 as isize,
	FMT_C = 5 as isize,
}
...
pub enum Param {
	...
	STRING = 3 as isize,
	MAT = 4 as isize,
	MAT_VECTOR = 5 as isize,
	ALGORITHM = 6 as isize,
	FLOAT = 7 as isize,
	UNSIGNED_INT = 8 as isize,
	UINT64 = 9 as isize,
	UCHAR = 11 as isize,
	SCALAR = 12 as isize,
}
...
pub enum UMatData_MemoryFlag {
	COPY_ON_MAP = 1 as isize,
	HOST_COPY_OBSOLETE = 2 as isize,
	DEVICE_COPY_OBSOLETE = 4 as isize,
	TEMP_UMAT = 8 as isize,
	TEMP_COPIED_UMAT = 24 as isize,
	USER_ALLOCATED = 32 as isize,
	DEVICE_MEM_MAPPED = 64 as isize,
	ASYNC_CLEANUP = 128 as isize,
}
...
pub enum _InputArray_KindFlag {
	KIND_SHIFT = 16 as isize,
	FIXED_TYPE = -2147483648 as isize,
	FIXED_SIZE = 1073741824 as isize,
	KIND_MASK = 2031616 as isize,
	NONE = 0 as isize,
	MAT = 65536 as isize,
	MATX = 131072 as isize,
	STD_VECTOR = 196608 as isize,
	STD_VECTOR_VECTOR = 262144 as isize,
	STD_VECTOR_MAT = 327680 as isize,
	EXPR = 393216 as isize,
	OPENGL_BUFFER = 458752 as isize,
	CUDA_HOST_MEM = 524288 as isize,
	CUDA_GPU_MAT = 589824 as isize,
	UMAT = 655360 as isize,
	STD_VECTOR_UMAT = 720896 as isize,
	STD_BOOL_VECTOR = 786432 as isize,
	STD_VECTOR_CUDA_GPU_MAT = 851968 as isize,
	STD_ARRAY = 917504 as isize,
	STD_ARRAY_MAT = 983040 as isize,
}
...
pub enum _OutputArray_DepthMask {
	DEPTH_MASK_8U = 1 as isize,
	DEPTH_MASK_8S = 2 as isize,
	DEPTH_MASK_16U = 4 as isize,
	DEPTH_MASK_16S = 8 as isize,
	DEPTH_MASK_32S = 16 as isize,
	DEPTH_MASK_32F = 32 as isize,
	DEPTH_MASK_64F = 64 as isize,
	DEPTH_MASK_16F = 128 as isize,
	DEPTH_MASK_ALL = 127 as isize,
	DEPTH_MASK_ALL_BUT_8S = 125 as isize,
	DEPTH_MASK_ALL_16F = 255 as isize,
	DEPTH_MASK_FLT = 96 as isize,
}
...
pub fn psnr(src1: &dyn core::ToInputArray, src2: &dyn core::ToInputArray, r: f64) -> Result<f64> {
	...
	unsafe { sys::cv_PSNR_const__InputArrayX_const__InputArrayX_double(src1.as_raw__InputArray(), src2.as_raw__InputArray(), r) }.into_result()
}
...
pub fn copy_to(src: &dyn core::ToInputArray, dst: &mut dyn core::ToOutputArray, mask: &dyn core::ToInputArray) -> Result<()> {
	...
	output_array_arg!(dst);
	input_array_arg!(mask);
	unsafe { sys::cv_copyTo_const__InputArrayX_const__OutputArrayX_const__InputArrayX(src.as_raw__InputArray(), dst.as_raw__OutputArray(), mask.as_raw__InputArray()) }.into_result()
}
...
pub fn solve_lp(func: &dyn core::ToInputArray, constr: &dyn core::ToInputArray, z: &mut dyn core::ToOutputArray) -> Result<i32> {
	...
	output_array_arg!(z);
	unsafe { sys::cv_solveLP_const__InputArrayX_const__InputArrayX_const__OutputArrayX(func.as_raw__InputArray(), constr.as_raw__InputArray(), z.as_raw__OutputArray()) }.into_result()
}
...
pub trait FileNodeTrait {
	...
	fn read_raw(&self, fmt: &str, vec: *mut c_void, len: size_t) -> Result<()> {
		...
		unsafe { sys::cv_FileNode_readRaw_const_const_StringX_voidX_size_t(self.as_raw_FileNode(), fmt.opencv_to_extern(), vec, len) }.into_result()
	}
...
pub trait FileNodeIteratorTrait {
	...
	fn read_raw(&mut self, fmt: &str, vec: *mut c_void, len: size_t) -> Result<core::FileNodeIterator> {
		extern_container_arg!(fmt);
		unsafe { sys::cv_FileNodeIterator_readRaw_const_StringX_voidX_size_t(self.as_raw_mut_FileNodeIterator(), fmt.opencv_to_extern(), vec, len) }.into_result().map(|r| unsafe { core::FileNodeIterator::opencv_from_extern(r) } )
	}
	...
	fn equal_to(&self, it: &core::FileNodeIterator) -> Result<bool> {
		unsafe { sys::cv_FileNodeIterator_equalTo_const_const_FileNodeIteratorX(self.as_raw_FileNodeIterator(), it.as_raw_FileNodeIterator()) }.into_result()
	}
...
impl FileNodeIterator {
	...
	pub fn new(node: &core::FileNode, seek_end: bool) -> Result<core::FileNodeIterator> {
		unsafe { sys::cv_FileNodeIterator_FileNodeIterator_const_FileNodeX_bool(node.as_raw_FileNode(), seek_end) }.into_result().map(|r| unsafe { core::FileNodeIterator::opencv_from_extern(r) } )
	}
...
pub trait FileStorageTrait {
	...
	fn write_mat(&mut self, name: &str, val: &core::Mat) -> Result<()> {
		extern_container_arg!(name);
		unsafe { sys::cv_FileStorage_write_const_StringX_const_MatX(self.as_raw_mut_FileStorage(), name.opencv_to_extern(), val.as_raw_Mat()) }.into_result()
	}
	...
	fn write_str_vec(&mut self, name: &str, val: &core::Vector::<String>) -> Result<()> {
		extern_container_arg!(name);
		unsafe { sys::cv_FileStorage_write_const_StringX_const_vector_String_X(self.as_raw_mut_FileStorage(), name.opencv_to_extern(), val.as_raw_VectorOfString()) }.into_result()
	}
	...
	fn write_raw(&mut self, fmt: &str, vec: *const c_void, len: size_t) -> Result<()> {
		extern_container_arg!(fmt);
		unsafe { sys::cv_FileStorage_writeRaw_const_StringX_const_voidX_size_t(self.as_raw_mut_FileStorage(), fmt.opencv_to_extern(), vec, len) }.into_result()
	}
	...
	fn start_write_struct(&mut self, name: &str, flags: i32, type_name: &str) -> Result<()> {
		...
		unsafe { sys::cv_FileStorage_startWriteStruct_const_StringX_int_const_StringX(self.as_raw_mut_FileStorage(), name.opencv_to_extern(), flags, type_name.opencv_to_extern()) }.into_result()
	}
...
impl LogTag {
	pub fn as_raw_LogTag(&self) -> *const c_void { self.as_raw() }
	pub fn as_raw_mut_LogTag(&mut self) -> *mut c_void { self.as_raw_mut() }
}

unsafe impl Send for LogTag {}

impl core::LogTagTrait for LogTag {
	fn as_raw_LogTag(&self) -> *const c_void { self.as_raw() }
	fn as_raw_mut_LogTag(&mut self) -> *mut c_void { self.as_raw_mut() }
}
