...
pub fn get_perspective_transform_slice(src: &[core::Point2f], dst: &[core::Point2f], solve_method: i32) -> Result<core::Mat> {
	unsafe { sys::cv_getPerspectiveTransform_const_Point2fX_const_Point2fX_int(src.as_ptr(), dst.as_ptr(), solve_method) }.into_result().map(|ptr| unsafe { core::Mat::from_raw(ptr) })
}
...
pub fn get_perspective_transform(src: &dyn core::ToInputArray, dst: &dyn core::ToInputArray, solve_method: i32) -> Result<core::Mat> {
	input_array_arg!(src);
	input_array_arg!(dst);
	unsafe { sys::cv_getPerspectiveTransform_const__InputArrayX_const__InputArrayX_int(src.as_raw__InputArray(), dst.as_raw__InputArray(), solve_method) }.into_result().map(|ptr| unsafe { core::Mat::from_raw(ptr) })
}
