...
use crate::type_ref::ConstnessOverride;
use crate::{GeneratorEnv, TypeRef};
...
impl<'tu, 'ge> ReturnTypeWrapper<'tu, 'ge> {
	...
	pub fn definition_location(&self) -> Cow<str> {
		match &self.definition_location {
			DefinitionLocation::Module => self.gen_env.module().into(),
			DefinitionLocation::Type => self.type_ref.rust_module(),
			DefinitionLocation::Custom(module) => module.into(),
		}
	}
}
