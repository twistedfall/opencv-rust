...
pub const CV_MAJOR_VERSION: i32 = 4;
pub const CV_MAT_CONT_FLAG_SHIFT: i32 = 14;
pub const CV_MINOR_VERSION: i32 = 1;
pub const CV_SUBMAT_FLAG_SHIFT: i32 = 15;
pub const CV_SUBMINOR_VERSION: i32 = 2;
...
pub enum AccessFlag {
    ACCESS_READ = ACCESS_READ as isize,
    ACCESS_WRITE = ACCESS_WRITE as isize,
    ACCESS_RW = ACCESS_RW as isize,
    // ACCESS_MASK = ACCESS_MASK as isize, // ignored discriminant
    ACCESS_FAST = ACCESS_FAST as isize,
}
...
pub enum Formatter_FormatType {
    FMT_DEFAULT = Formatter_FMT_DEFAULT as isize,
    FMT_MATLAB = Formatter_FMT_MATLAB as isize,
    FMT_CSV = Formatter_FMT_CSV as isize,
    FMT_PYTHON = Formatter_FMT_PYTHON as isize,
    FMT_NUMPY = Formatter_FMT_NUMPY as isize,
    FMT_C = Formatter_FMT_C as isize,
}
...
pub fn psnr(src1: &dyn core::ToInputArray, src2: &dyn core::ToInputArray, r: f64) -> Result<f64> {
    ...
    unsafe { sys::cv_PSNR__InputArray__InputArray_double(src1.as_raw__InputArray(), src2.as_raw__InputArray(), r) }.into_result()
}
...
pub fn copy_to(src: &dyn core::ToInputArray, dst: &mut dyn core::ToOutputArray, mask: &dyn core::ToInputArray) -> Result<()> {
    ...
    output_array_arg!(dst);
    input_array_arg!(mask);
    unsafe { sys::cv_copyTo__InputArray__OutputArray__InputArray(src.as_raw__InputArray(), dst.as_raw__OutputArray(), mask.as_raw__InputArray()) }.into_result()
}
...
/// ![Graphs for the different norm functions from the above example](https://docs.opencv.org/4.1.2/NormTypes_OneArray_1-2-INF.png)
///
...
pub fn solve_lp(func: &dyn core::ToInputArray, constr: &dyn core::ToInputArray, z: &mut dyn core::ToOutputArray) -> Result<i32> {
    ...
    output_array_arg!(z);
    unsafe { sys::cv_solveLP__InputArray__InputArray__OutputArray(func.as_raw__InputArray(), constr.as_raw__InputArray(), z.as_raw__OutputArray()) }.into_result()
}
...
pub fn get_log_tag_level(tag: &str) -> Result<core::LogLevel> {
    string_arg!(tag);
    unsafe { sys::cv_utils_logging_getLogTagLevel_const_char_X(tag.as_ptr()) }.into_result()
}
...
pub fn write_log_message_ex(log_level: core::LogLevel, tag: &str, file: &str, line: i32, func: &str, message: &str) -> Result<()> {
    ...
    string_arg!(func);
    string_arg!(message);
    unsafe { sys::cv_utils_logging_internal_writeLogMessageEx_LogLevel_const_char_X_const_char_X_int_const_char_X_const_char_X(log_level, tag.as_ptr(), file.as_ptr(), line, func.as_ptr(), message.as_ptr()) }.into_result()
}
...
pub fn register_log_tag(plogtag: &mut core::LogTag) -> Result<()> {
    unsafe { sys::cv_utils_logging_registerLogTag_LogTag(plogtag.as_raw_LogTag()) }.into_result()
}
...
pub fn set_log_tag_level(tag: &str, level: core::LogLevel) -> Result<()> {
    string_arg!(tag);
    unsafe { sys::cv_utils_logging_setLogTagLevel_const_char_X_LogLevel(tag.as_ptr(), level) }.into_result()
}
...
impl dyn Formatter + '_ {
    ...
    pub fn get(fmt: core::Formatter_FormatType) -> Result<types::PtrOfFormatter> {
        unsafe { sys::cv_Formatter_get_Formatter_FormatType(fmt) }.into_result().map(|ptr| types::PtrOfFormatter { ptr })
    }
...
impl Mat {
    ...
    pub fn get_umat(&self, access_flags: core::AccessFlag, usage_flags: core::UMatUsageFlags) -> Result<core::UMat> {
...
/// ![image](https://docs.opencv.org/4.1.2/rotatedrect.png)
///
/// ## See also
...
impl UMat {
    ...
    pub fn get_mat(&self, flags: core::AccessFlag) -> Result<core::Mat> {
...
impl Drop for LogTag {
    fn drop(&mut self) {
        unsafe { sys::cv_LogTag_delete(self.ptr) };
    }
}

impl LogTag {
    #[inline(always)] pub fn as_raw_LogTag(&self) -> *mut c_void { self.ptr }
...
impl LogTag {
    pub fn new(_name: &str, _level: core::LogLevel) -> Result<core::LogTag> {
        string_arg!(_name);
        unsafe { sys::cv_utils_logging_LogTag_LogTag_const_char_X_LogLevel(_name.as_ptr(), _level) }.into_result().map(|ptr| core::LogTag { ptr })
    }
...
pub static CV_VERSION: &'static str = "4.1.2";
