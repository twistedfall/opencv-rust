...
pub fn estimate_pose_single_markers(corners: &dyn core::ToInputArray, marker_length: f32, camera_matrix: &dyn core::ToInputArray, dist_coeffs: &dyn core::ToInputArray, rvecs: &mut dyn core::ToOutputArray, tvecs: &mut dyn core::ToOutputArray) -> Result<()> {
    ...
    output_array_arg!(rvecs);
    output_array_arg!(tvecs);
    unsafe { sys::cv_aruco_estimatePoseSingleMarkers__InputArray_float__InputArray__InputArray__OutputArray__OutputArray(corners.as_raw__InputArray(), marker_length, camera_matrix.as_raw__InputArray(), dist_coeffs.as_raw__InputArray(), rvecs.as_raw__OutputArray(), tvecs.as_raw__OutputArray()) }.into_result()
}

/// @see generateCustomDictionary
pub fn generate_custom_dictionary(n_markers: i32, marker_size: i32) -> Result<types::PtrOfDictionary> {
...
pub fn generate_custom_dictionary_with_base(n_markers: i32, marker_size: i32, base_dictionary: &types::PtrOfDictionary) -> Result<types::PtrOfDictionary> {
    unsafe { sys::cv_aruco_generateCustomDictionary_int_int_PtrOfDictionary(n_markers, marker_size, base_dictionary.as_raw_PtrOfDictionary()) }.into_result().map(|ptr| types::PtrOfDictionary { ptr })
}
...
impl Dictionary {
    ...
    /// @see generateCustomDictionary
    pub fn create_with_base(n_markers: i32, marker_size: i32, base_dictionary: &types::PtrOfDictionary) -> Result<types::PtrOfDictionary> {
        unsafe { sys::cv_aruco_Dictionary_create_int_int_PtrOfDictionary(n_markers, marker_size, base_dictionary.as_raw_PtrOfDictionary()) }.into_result().map(|ptr| types::PtrOfDictionary { ptr })
    }
