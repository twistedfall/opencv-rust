...
pub trait HDF5 {
    ...
    fn dscreate(&self, rows: i32, cols: i32, _type: i32, dslabel: &str) -> Result<()> {
        ...
        unsafe { sys::cv_hdf_HDF5_dscreate_const_int_int_int_String(self.as_raw_HDF5(), rows, cols, _type, dslabel.as_ptr() as _) }.into_result()
    }
    
    fn dscreate_1(&self, rows: i32, cols: i32, _type: i32, dslabel: &str, compresslevel: i32) -> Result<()> {
        string_arg!(mut dslabel);
        unsafe { sys::cv_hdf_HDF5_dscreate_const_int_int_int_String_int(self.as_raw_HDF5(), rows, cols, _type, dslabel.as_ptr() as _, compresslevel) }.into_result()
    }
    
    fn dscreate_2(&self, rows: i32, cols: i32, _type: i32, dslabel: &str, compresslevel: i32, dims_chunks: &types::VectorOfint) -> Result<()> {
        string_arg!(mut dslabel);
        unsafe { sys::cv_hdf_HDF5_dscreate_const_int_int_int_String_int_VectorOfint(self.as_raw_HDF5(), rows, cols, _type, dslabel.as_ptr() as _, compresslevel, dims_chunks.as_raw_VectorOfint()) }.into_result()
    }
    ...
    fn dscreate_3(&self, rows: i32, cols: i32, _type: i32, dslabel: &str, compresslevel: i32, dims_chunks: &i32) -> Result<()> {
        string_arg!(mut dslabel);
        unsafe { sys::cv_hdf_HDF5_dscreate_const_int_int_int_String_int_const_int_X(self.as_raw_HDF5(), rows, cols, _type, dslabel.as_ptr() as _, compresslevel, dims_chunks) }.into_result()
    }
    
    fn dscreate_4(&self, n_dims: i32, sizes: &i32, _type: i32, dslabel: &str) -> Result<()> {
        string_arg!(mut dslabel);
        unsafe { sys::cv_hdf_HDF5_dscreate_const_int_const_int_X_int_String(self.as_raw_HDF5(), n_dims, sizes, _type, dslabel.as_ptr() as _) }.into_result()
    }
    
    fn dscreate_5(&self, n_dims: i32, sizes: &i32, _type: i32, dslabel: &str, compresslevel: i32) -> Result<()> {
        string_arg!(mut dslabel);
        unsafe { sys::cv_hdf_HDF5_dscreate_const_int_const_int_X_int_String_int(self.as_raw_HDF5(), n_dims, sizes, _type, dslabel.as_ptr() as _, compresslevel) }.into_result()
    }
    ...
    fn dscreate_6(&self, sizes: &types::VectorOfint, _type: i32, dslabel: &str, compresslevel: i32, dims_chunks: &types::VectorOfint) -> Result<()> {
        string_arg!(mut dslabel);
        unsafe { sys::cv_hdf_HDF5_dscreate_const_VectorOfint_int_String_int_VectorOfint(self.as_raw_HDF5(), sizes.as_raw_VectorOfint(), _type, dslabel.as_ptr() as _, compresslevel, dims_chunks.as_raw_VectorOfint()) }.into_result()
    }
    ...
    fn dscreate_7(&self, n_dims: i32, sizes: &i32, _type: i32, dslabel: &str, compresslevel: i32, dims_chunks: &i32) -> Result<()> {
        string_arg!(mut dslabel);
        unsafe { sys::cv_hdf_HDF5_dscreate_const_int_const_int_X_int_String_int_const_int_X(self.as_raw_HDF5(), n_dims, sizes, _type, dslabel.as_ptr() as _, compresslevel, dims_chunks) }.into_result()
    }
    ...
    fn dsgetsize(&self, dslabel: &str, dims_flag: i32) -> Result<types::VectorOfint> {
        string_arg!(mut dslabel);
        unsafe { sys::cv_hdf_HDF5_dsgetsize_const_String_int(self.as_raw_HDF5(), dslabel.as_ptr() as _, dims_flag) }.into_result().map(|ptr| types::VectorOfint { ptr })
    }
    ...
    fn kpwrite(&self, keypoints: &types::VectorOfKeyPoint, kplabel: &str, offset: i32, counts: i32) -> Result<()> {
        string_arg!(mut kplabel);
        unsafe { sys::cv_hdf_HDF5_kpwrite_const_VectorOfKeyPoint_String_int_int(self.as_raw_HDF5(), keypoints.as_raw_VectorOfKeyPoint(), kplabel.as_ptr() as _, offset, counts) }.into_result()
    }
    ...
    fn kpinsert(&self, keypoints: &types::VectorOfKeyPoint, kplabel: &str, offset: i32, counts: i32) -> Result<()> {
        string_arg!(mut kplabel);
        unsafe { sys::cv_hdf_HDF5_kpinsert_const_VectorOfKeyPoint_String_int_int(self.as_raw_HDF5(), keypoints.as_raw_VectorOfKeyPoint(), kplabel.as_ptr() as _, offset, counts) }.into_result()
    }
    ...
    fn kpread(&self, keypoints: &mut types::VectorOfKeyPoint, kplabel: &str, offset: i32, counts: i32) -> Result<()> {
        string_arg!(mut kplabel);
        unsafe { sys::cv_hdf_HDF5_kpread_const_VectorOfKeyPoint_String_int_int(self.as_raw_HDF5(), keypoints.as_raw_VectorOfKeyPoint(), kplabel.as_ptr() as _, offset, counts) }.into_result()
    }
