...
pub fn check_fixed(g: &mut core::Mat, flags: i32, n: i32) -> Result<()> {
    unsafe { sys::cv_omnidir_internal_checkFixed_Mat_int_int(g.as_raw_Mat(), flags, n) }.into_result()
}
...
impl CustomPattern {
    ...
    pub fn get_pattern_points(&mut self, original_points: &mut dyn core::ToOutputArray) -> Result<()> {
        output_array_arg!(original_points);
        unsafe { sys::cv_ccalib_CustomPattern_getPatternPoints__OutputArray(self.as_raw_CustomPattern(), original_points.as_raw__OutputArray()) }.into_result()
    }
