...
/// floating-point vector <span lang='latex'>(x, y, radius)</span> .
...
/// <span lang='latex'>(\rho, \theta)</span> . <span lang='latex'>\rho</span> is the distance from the coordinate origin <span lang='latex'>(0,0)</span> (top-left corner of
...
/// is equivalent to
///
/// <div lang='latex'>\texttt{Sobel(src, dst, ddepth, dx, dy, CV\_SCHARR, scale, delta, borderType)} .</div>
///
/// ## Parameters
/// * src: input image.
...
///
/// <div lang='latex'>\sum _i(DI_{p_i}  \cdot {DI_{p_i}}^T) -  \sum _i(DI_{p_i}  \cdot {DI_{p_i}}^T  \cdot p_i)</div>
///
...
/// `ksize=CV_SCHARR`, the Scharr <span lang='latex'>3 \times 3</span> kernels are generated (see cv::Scharr). Otherwise, Sobel
...
///
/// <div lang='latex'>dst(x, y) = src(x +  \texttt{center.x} - ( \texttt{dst.cols} -1)*0.5, y +  \texttt{center.y} - ( \texttt{dst.rows} -1)*0.5)</div>
///
...
/// [KleeLaskowski85](https://docs.opencv.org/3.2.0/d0/de3/citelist.html#CITEREF_KleeLaskowski85) papers. O'Rourke provides a <span lang='latex'>\theta(n)</span> algorithm for finding the minimal
