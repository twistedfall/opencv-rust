...
///   <span lang='latex'>\texttt{mtx}_{ij}=\texttt{mtx}_{ji}</span> for <span lang='latex'>i > j</span> if
/// lowerToUpper=false
///   <span lang='latex'>\texttt{mtx}_{ij}=\texttt{mtx}_{ji}</span> for <span lang='latex'>i < j</span> if
/// lowerToUpper=true
/// ## Parameters
...
pub fn complete_symm(mtx: &mut core::Mat, lower_to_upper: bool) -> Result<()> {
    unsafe { sys::cv_completeSymm_Mat_bool(mtx.as_raw_Mat(), lower_to_upper) }.into_result()
}
...
/// and groups the input samples around the clusters. As an output, <span lang='latex'>\texttt{labels}_i</span> contains a
...
/// src2 ):
///
/// <div lang='latex'>norm =  \forkthree{\|\texttt{src1}\|_{L_{\infty}} =  \max _I | \texttt{src1} (I)|}{if  \(\texttt{normType} = \texttt{NORM_INF}\) }
/// { \| \texttt{src1} \| _{L_1} =  \sum _I | \texttt{src1} (I)|}{if  \(\texttt{normType} = \texttt{NORM_L1}\) }
/// { \| \texttt{src1} \| _{L_2} =  \sqrt{\sum_I \texttt{src1}(I)^2} }{if  \(\texttt{normType} = \texttt{NORM_L2}\) }</div>
///
/// or an absolute or relative difference norm if src2 is there:
///
/// <div lang='latex'>norm =  \forkthree{\|\texttt{src1}-\texttt{src2}\|_{L_{\infty}} =  \max _I | \texttt{src1} (I) -  \texttt{src2} (I)|}{if  \(\texttt{normType} = \texttt{NORM_INF}\) }
/// { \| \texttt{src1} - \texttt{src2} \| _{L_1} =  \sum _I | \texttt{src1} (I) -  \texttt{src2} (I)|}{if  \(\texttt{normType} = \texttt{NORM_L1}\) }
/// { \| \texttt{src1} - \texttt{src2} \| _{L_2} =  \sqrt{\sum_I (\texttt{src1}(I) - \texttt{src2}(I))^2} }{if  \(\texttt{normType} = \texttt{NORM_L2}\) }</div>
///
/// or
///
/// <div lang='latex'>norm =  \forkthree{\frac{\|\texttt{src1}-\texttt{src2}\|_{L_{\infty}}    }{\|\texttt{src2}\|_{L_{\infty}} }}{if  \(\texttt{normType} = \texttt{NORM_RELATIVE_INF}\) }
/// { \frac{\|\texttt{src1}-\texttt{src2}\|_{L_1} }{\|\texttt{src2}\|_{L_1}} }{if  \(\texttt{normType} = \texttt{NORM_RELATIVE_L1}\) }
/// { \frac{\|\texttt{src1}-\texttt{src2}\|_{L_2} }{\|\texttt{src2}\|_{L_2}} }{if  \(\texttt{normType} = \texttt{NORM_RELATIVE_L2}\) }</div>
///
...
/// and the remaining to <span lang='latex'>A</span>. It should containt 32- or 64-bit floating point numbers.
