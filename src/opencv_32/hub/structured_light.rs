...
impl dyn SinusoidalPattern + '_ {
    ...
    pub fn create(parameters: &crate::structured_light::SinusoidalPattern_Params) -> Result<types::PtrOfSinusoidalPattern> {
        unsafe { sys::cv_structured_light_SinusoidalPattern_create_Params(parameters.as_raw_SinusoidalPattern_Params()) }.into_result().map(|ptr| types::PtrOfSinusoidalPattern { ptr })
    }
...
pub trait StructuredLightPattern: core::Algorithm {
    ...
    fn decode(&self, pattern_images: &types::VectorOfMat, disparity_map: &mut core::Mat, black_images: &types::VectorOfMat, white_images: &types::VectorOfMat, flags: i32) -> Result<bool> {
        unsafe { sys::cv_structured_light_StructuredLightPattern_decode_const_VectorOfMat_Mat_VectorOfMat_VectorOfMat_int(self.as_raw_StructuredLightPattern(), pattern_images.as_raw_VectorOfMat(), disparity_map.as_raw_Mat(), black_images.as_raw_VectorOfMat(), white_images.as_raw_VectorOfMat(), flags) }.into_result()
    }
