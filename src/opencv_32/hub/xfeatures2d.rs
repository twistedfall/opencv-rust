...
// boxed class cv::xfeatures2d::BoostDesc
...
pub trait VGG: crate::features2d::Feature2DTrait {
    ...
    fn compute(&mut self, image: &dyn core::ToInputArray, keypoints: &mut types::VectorOfKeyPoint, descriptors: &mut dyn core::ToOutputArray) -> Result<()> {
        input_array_arg!(image);
        output_array_arg!(descriptors);
        unsafe { sys::cv_xfeatures2d_VGG_compute__InputArray_VectorOfKeyPoint__OutputArray(self.as_raw_VGG(), image.as_raw__InputArray(), keypoints.as_raw_VectorOfKeyPoint(), descriptors.as_raw__OutputArray()) }.into_result()
    }
