...
pub trait SuperResolution: core::AlgorithmTrait + crate::superres::FrameSource {
    ...
    fn get_lambda(&self) -> Result<f64> {
        unsafe { sys::cv_superres_SuperResolution_getLambda_const(self.as_raw_SuperResolution()) }.into_result()
    }
    
    /// @copybrief getLambda @see getLambda
    fn set_lambda(&mut self, val: f64) -> Result<()> {
        unsafe { sys::cv_superres_SuperResolution_setLambda_double(self.as_raw_SuperResolution(), val) }.into_result()
    }
