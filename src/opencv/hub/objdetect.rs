...
//! ----------------------------------------------------------
//! 
//! The object detector described below has been initially proposed by Paul Viola [Viola01](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_Viola01) and
//! improved by Rainer Lienhart [Lienhart02](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_Lienhart02) .
//! 
...
//! Haar-like features:
//! 
//! ![image](https://docs.opencv.org/4.2.0/haarfeatures.png)
//! 
...
/// 
/// the HOG descriptor algorithm introduced by Navneet Dalal and Bill Triggs [Dalal2005](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_Dalal2005) .
/// 
/// useful links:
/// 
...
pub trait HOGDescriptorTrait {
	...
	fn set_histogram_norm_type(&mut self, val: crate::objdetect::HOGDescriptor_HistogramNormType) -> () {
		unsafe { sys::cv_HOGDescriptor_setHistogramNormType_HistogramNormType(self.as_raw_HOGDescriptor(), val) }.into_result().expect("Infallible function failed: set_histogram_norm_type")
	}
	...
	fn set_svm_detector(&mut self, svmdetector: &dyn core::ToInputArray) -> Result<()> {
		...
		unsafe { sys::cv_HOGDescriptor_setSVMDetector_const__InputArrayX(self.as_raw_HOGDescriptor(), svmdetector.as_raw__InputArray()) }.into_result()
	}
	...
	fn detect_weights(&self, img: &dyn core::ToInputArray, found_locations: &mut types::VectorOfPoint, weights: &mut types::VectorOff64, hit_threshold: f64, win_stride: core::Size, padding: core::Size, search_locations: &types::VectorOfPoint) -> Result<()> {
		input_array_arg!(img);
		unsafe { sys::cv_HOGDescriptor_detect_const_const__InputArrayX_vector_Point_X_vector_double_X_double_Size_Size_const_vector_Point_X(self.as_raw_HOGDescriptor(), img.as_raw__InputArray(), found_locations.as_raw_VectorOfPoint(), weights.as_raw_VectorOff64(), hit_threshold, &win_stride, &padding, search_locations.as_raw_VectorOfPoint()) }.into_result()
	}
	...
	/// * padding: Size()
	/// * search_locations: std::vector<Point>()
	fn detect(&self, img: &dyn core::ToInputArray, found_locations: &mut types::VectorOfPoint, hit_threshold: f64, win_stride: core::Size, padding: core::Size, search_locations: &types::VectorOfPoint) -> Result<()> {
		input_array_arg!(img);
		unsafe { sys::cv_HOGDescriptor_detect_const_const__InputArrayX_vector_Point_X_double_Size_Size_const_vector_Point_X(self.as_raw_HOGDescriptor(), img.as_raw__InputArray(), found_locations.as_raw_VectorOfPoint(), hit_threshold, &win_stride, &padding, search_locations.as_raw_VectorOfPoint()) }.into_result()
	}
	...
	/// * win_stride: Size()
	/// * padding: Size()
	fn detect_roi(&self, img: &dyn core::ToInputArray, locations: &types::VectorOfPoint, found_locations: &mut types::VectorOfPoint, confidences: &mut types::VectorOff64, hit_threshold: f64, win_stride: core::Size, padding: core::Size) -> Result<()> {
		input_array_arg!(img);
		unsafe { sys::cv_HOGDescriptor_detectROI_const_const__InputArrayX_const_vector_Point_X_vector_Point_X_vector_double_X_double_Size_Size(self.as_raw_HOGDescriptor(), img.as_raw__InputArray(), locations.as_raw_VectorOfPoint(), found_locations.as_raw_VectorOfPoint(), confidences.as_raw_VectorOff64(), hit_threshold, &win_stride, &padding) }.into_result()
	}
	...
	/// * hit_threshold: 0
	/// * group_threshold: 0
	fn detect_multi_scale_roi(&self, img: &dyn core::ToInputArray, found_locations: &mut types::VectorOfRect, locations: &mut types::VectorOfDetectionROI, hit_threshold: f64, group_threshold: i32) -> Result<()> {
		input_array_arg!(img);
		unsafe { sys::cv_HOGDescriptor_detectMultiScaleROI_const_const__InputArrayX_vector_Rect_X_vector_DetectionROI_X_double_int(self.as_raw_HOGDescriptor(), img.as_raw__InputArray(), found_locations.as_raw_VectorOfRect(), locations.as_raw_VectorOfDetectionROI(), hit_threshold, group_threshold) }.into_result()
	}
