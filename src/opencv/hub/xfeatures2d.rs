...
///       ACM, 2010.
/// [BeecksUS10](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_BeecksUS10)
/// 
...
pub fn fast_for_point_set(image: &dyn core::ToInputArray, keypoints: &mut types::VectorOfKeyPoint, threshold: i32, nonmax_suppression: bool, typ: crate::features2d::FastFeatureDetector_DetectorType) -> Result<()> {
	input_array_arg!(image);
	unsafe { sys::cv_xfeatures2d_FASTForPointSet_const__InputArrayX_vector_KeyPoint_X_int_bool_DetectorType(image.as_raw__InputArray(), keypoints.as_raw_VectorOfKeyPoint(), threshold, nonmax_suppression, typ) }.into_result()
}

/// GMS (Grid-based Motion Statistics) feature matching strategy described in [Bian2017gms](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_Bian2017gms) .
/// ## Parameters
...
/// LOGOS (Local geometric support for high-outlier spatial verification) feature matching strategy described in [Lowry2018LOGOSLG](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_Lowry2018LOGOSLG) .
/// ## Parameters
/// * keypoints1: Input keypoints of image1.
/// * keypoints2: Input keypoints of image2.
...
pub fn match_logos(keypoints1: &types::VectorOfKeyPoint, keypoints2: &types::VectorOfKeyPoint, nn1: &types::VectorOfi32, nn2: &types::VectorOfi32, matches1to2: &mut types::VectorOfDMatch) -> Result<()> {
	unsafe { sys::cv_xfeatures2d_matchLOGOS_const_vector_KeyPoint_X_const_vector_KeyPoint_X_const_vector_int_X_const_vector_int_X_vector_DMatch_X(keypoints1.as_raw_VectorOfKeyPoint(), keypoints2.as_raw_VectorOfKeyPoint(), nn1.as_raw_VectorOfi32(), nn2.as_raw_VectorOfi32(), matches1to2.as_raw_VectorOfDMatch()) }.into_result()
}
...
pub trait AffineFeature2D: core::AlgorithmTrait + crate::features2d::Feature2DTrait {
	...
	fn detect(&mut self, image: &dyn core::ToInputArray, keypoints: &mut types::VectorOfElliptic_KeyPoint, mask: &dyn core::ToInputArray) -> Result<()> {
		...
		unsafe { sys::cv_xfeatures2d_AffineFeature2D_detect_const__InputArrayX_vector_Elliptic_KeyPoint_X_const__InputArrayX(self.as_raw_AffineFeature2D(), image.as_raw__InputArray(), keypoints.as_raw_VectorOfElliptic_KeyPoint(), mask.as_raw__InputArray()) }.into_result()
	}
...
impl dyn AffineFeature2D + '_ {
	...
	pub fn create_with_extractor(keypoint_detector: types::PtrOfFeatureDetector, descriptor_extractor: types::PtrOfDescriptorExtractor) -> Result<types::PtrOfAffineFeature2D> {
		unsafe { sys::cv_xfeatures2d_AffineFeature2D_create_Ptr_FeatureDetector__Ptr_DescriptorExtractor_(keypoint_detector.as_raw_PtrOfFeatureDetector(), descriptor_extractor.as_raw_PtrOfDescriptorExtractor()) }.into_result().map(|ptr| types::PtrOfAffineFeature2D { ptr })
	}
	
	/// Creates an instance where keypoint detector and descriptor
	/// extractor are identical.
	pub fn create(keypoint_detector: types::PtrOfFeatureDetector) -> Result<types::PtrOfAffineFeature2D> {
		unsafe { sys::cv_xfeatures2d_AffineFeature2D_create_Ptr_FeatureDetector_(keypoint_detector.as_raw_PtrOfFeatureDetector()) }.into_result().map(|ptr| types::PtrOfAffineFeature2D { ptr })
	}
	
}
/// Class implementing BoostDesc (Learning Image Descriptors with Boosting), described in
/// [Trzcinski13a](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_Trzcinski13a) and [Trzcinski13b](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_Trzcinski13b).
/// 
/// ## Parameters
...
/// samples subfolder.
pub trait BoostDesc: core::AlgorithmTrait + crate::features2d::Feature2DTrait {
...
/// Class for computing BRIEF descriptors described in [calon2010](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_calon2010) .
/// 
/// ## Parameters
...
/// Class implementing DAISY descriptor, described in [Tola10](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_Tola10)
/// 
/// ## Parameters
...
impl dyn DAISY + '_ {
	...
	pub fn create(radius: f32, q_radius: i32, q_theta: i32, q_hist: i32, norm: crate::xfeatures2d::DAISY_NormalizationType, h: &dyn core::ToInputArray, interpolation: bool, use_orientation: bool) -> Result<types::PtrOfDAISY> {
		input_array_arg!(h);
		unsafe { sys::cv_xfeatures2d_DAISY_create_float_int_int_int_NormalizationType_const__InputArrayX_bool_bool(radius, q_radius, q_theta, q_hist, norm, h.as_raw__InputArray(), interpolation, use_orientation) }.into_result().map(|ptr| types::PtrOfDAISY { ptr })
	}
	
}
/// Elliptic region around an interest point.
pub trait Elliptic_KeyPointTrait {
	fn as_raw_Elliptic_KeyPoint(&self) -> *mut c_void;
	...
	fn set_axes(&mut self, val: core::Size_<f32>) -> () {
		unsafe { sys::cv_xfeatures2d_Elliptic_KeyPoint_setAxes_Size__float_(self.as_raw_Elliptic_KeyPoint(), &val) }.into_result().expect("Infallible function failed: set_axes")
	}
...
impl Drop for Elliptic_KeyPoint {
	fn drop(&mut self) {
		...
		unsafe { cv_Elliptic_KeyPoint_delete(self.as_raw_Elliptic_KeyPoint()) };
	}
}
...
impl crate::xfeatures2d::Elliptic_KeyPointTrait for Elliptic_KeyPoint {
	fn as_raw_Elliptic_KeyPoint(&self) -> *mut c_void { self.ptr }
}
...
/// Class implementing the FREAK (*Fast Retina Keypoint*) keypoint descriptor, described in [AOV12](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_AOV12) .
/// 
...
pub trait FREAKTrait: core::AlgorithmTrait + crate::features2d::Feature2DTrait {
	...
	fn nb_scales(&self) -> i32 {
		unsafe { sys::cv_xfeatures2d_FREAK_NB_SCALES_const(self.as_raw_FREAK()) }.into_result().expect("Infallible function failed: nb_scales")
	}
	
	fn nb_pairs(&self) -> i32 {
		unsafe { sys::cv_xfeatures2d_FREAK_NB_PAIRS_const(self.as_raw_FREAK()) }.into_result().expect("Infallible function failed: nb_pairs")
	}
	
	fn nb_orienpairs(&self) -> i32 {
		unsafe { sys::cv_xfeatures2d_FREAK_NB_ORIENPAIRS_const(self.as_raw_FREAK()) }.into_result().expect("Infallible function failed: nb_orienpairs")
	}
...
/// Class implementing the Harris-Laplace feature detector as described in [Mikolajczyk2004](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_Mikolajczyk2004).
pub trait HarrisLaplaceFeatureDetectorTrait: core::AlgorithmTrait + crate::features2d::Feature2DTrait {
...
impl Drop for HarrisLaplaceFeatureDetector {
	fn drop(&mut self) {
		...
		unsafe { cv_HarrisLaplaceFeatureDetector_delete(self.as_raw_HarrisLaplaceFeatureDetector()) };
	}
}
...
impl crate::xfeatures2d::HarrisLaplaceFeatureDetectorTrait for HarrisLaplaceFeatureDetector {
	fn as_raw_HarrisLaplaceFeatureDetector(&self) -> *mut c_void { self.ptr }
}
...
/// Class implementing the locally uniform comparison image descriptor, described in [LUCID](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_LUCID)
/// 
...
/// Class implementing the MSD (*Maximal Self-Dissimilarity*) keypoint detector, described in [Tombari14](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_Tombari14).
/// 
...
///       as described in [KrulisLS16](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_KrulisLS16).
...
/// [KrulisLS16](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_KrulisLS16)
/// [BeecksUS10](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_BeecksUS10)
...
/// (SIFT) algorithm by D. Lowe [Lowe04](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_Lowe04) .
...
/// Class for extracting Speeded Up Robust Features from an image [Bay06](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_Bay06) .
/// 
...
/// The class implements the keypoint detector introduced by [Agrawal08](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_Agrawal08), synonym of StarDetector. :
...
/// using "Descriptor Learning Using Convex Optimisation" (DLCO) aparatus described in [Simonyan14](https://docs.opencv.org/4.2.0/d0/de3/citelist.html#CITEREF_Simonyan14).
/// 
/// ## Parameters
