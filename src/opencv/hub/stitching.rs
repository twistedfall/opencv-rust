...
/// ## C++ default parameters
/// * mask: noArray()
pub fn compute_image_features2(features_finder: &types::PtrOfFeature2D, image: &dyn core::ToInputArray, features: &mut crate::stitching::Detail_ImageFeatures, mask: &dyn core::ToInputArray) -> Result<()> {
	input_array_arg!(image);
	input_array_arg!(mask);
	unsafe { sys::cv_detail_computeImageFeatures_const_Ptr_Feature2D_X_const__InputArrayX_ImageFeaturesX_const__InputArrayX(features_finder.as_raw_PtrOfFeature2D(), image.as_raw__InputArray(), features.as_raw_Detail_ImageFeatures(), mask.as_raw__InputArray()) }.into_result()
}
...
/// ## C++ default parameters
/// * masks: noArray()
pub fn compute_image_features(features_finder: &types::PtrOfFeature2D, images: &dyn core::ToInputArray, features: &mut types::VectorOfDetail_ImageFeatures, masks: &dyn core::ToInputArray) -> Result<()> {
	input_array_arg!(images);
	input_array_arg!(masks);
	unsafe { sys::cv_detail_computeImageFeatures_const_Ptr_Feature2D_X_const__InputArrayX_vector_ImageFeatures_X_const__InputArrayX(features_finder.as_raw_PtrOfFeature2D(), images.as_raw__InputArray(), features.as_raw_VectorOfDetail_ImageFeatures(), masks.as_raw__InputArray()) }.into_result()
}
...
pub trait PyRotationWarperTrait {
	...
	fn warp_point(&mut self, pt: core::Point2f, k: &dyn core::ToInputArray, r: &dyn core::ToInputArray) -> Result<core::Point2f> {
		input_array_arg!(k);
		input_array_arg!(r);
		unsafe { sys::cv_PyRotationWarper_warpPoint_const_Point2fX_const__InputArrayX_const__InputArrayX(self.as_raw_PyRotationWarper(), &pt, k.as_raw__InputArray(), r.as_raw__InputArray()) }.into_result()
	}
	...
	fn build_maps(&mut self, src_size: core::Size, k: &dyn core::ToInputArray, r: &dyn core::ToInputArray, xmap: &mut dyn core::ToOutputArray, ymap: &mut dyn core::ToOutputArray) -> Result<core::Rect> {
		...
		unsafe { sys::cv_PyRotationWarper_buildMaps_Size_const__InputArrayX_const__InputArrayX_const__OutputArrayX_const__OutputArrayX(self.as_raw_PyRotationWarper(), &src_size, k.as_raw__InputArray(), r.as_raw__InputArray(), xmap.as_raw__OutputArray(), ymap.as_raw__OutputArray()) }.into_result()
	}
	...
	fn warp(&mut self, src: &dyn core::ToInputArray, k: &dyn core::ToInputArray, r: &dyn core::ToInputArray, interp_mode: i32, border_mode: i32, dst: &mut dyn core::ToOutputArray) -> Result<core::Point> {
		...
		unsafe { sys::cv_PyRotationWarper_warp_const__InputArrayX_const__InputArrayX_const__InputArrayX_int_int_const__OutputArrayX(self.as_raw_PyRotationWarper(), src.as_raw__InputArray(), k.as_raw__InputArray(), r.as_raw__InputArray(), interp_mode, border_mode, dst.as_raw__OutputArray()) }.into_result()
	}
	...
	fn warp_backward(&mut self, src: &dyn core::ToInputArray, k: &dyn core::ToInputArray, r: &dyn core::ToInputArray, interp_mode: i32, border_mode: i32, dst_size: core::Size, dst: &mut dyn core::ToOutputArray) -> Result<()> {
		...
		input_array_arg!(r);
		output_array_arg!(dst);
		unsafe { sys::cv_PyRotationWarper_warpBackward_const__InputArrayX_const__InputArrayX_const__InputArrayX_int_int_Size_const__OutputArrayX(self.as_raw_PyRotationWarper(), src.as_raw__InputArray(), k.as_raw__InputArray(), r.as_raw__InputArray(), interp_mode, border_mode, &dst_size, dst.as_raw__OutputArray()) }.into_result()
	}
	...
	fn warp_roi(&mut self, src_size: core::Size, k: &dyn core::ToInputArray, r: &dyn core::ToInputArray) -> Result<core::Rect> {
		input_array_arg!(k);
		input_array_arg!(r);
		unsafe { sys::cv_PyRotationWarper_warpRoi_Size_const__InputArrayX_const__InputArrayX(self.as_raw_PyRotationWarper(), &src_size, k.as_raw__InputArray(), r.as_raw__InputArray()) }.into_result()
	}
	...
	fn set_scale(&mut self, unnamed: f32) -> Result<()> {
		unsafe { sys::cv_PyRotationWarper_setScale_float(self.as_raw_PyRotationWarper(), unnamed) }.into_result()
	}
...
impl Drop for PyRotationWarper {
	fn drop(&mut self) {
		...
		unsafe { cv_PyRotationWarper_delete(self.as_raw_PyRotationWarper()) };
	}
}

impl PyRotationWarper {
	pub fn as_raw_PyRotationWarper(&self) -> *mut c_void { self.ptr }
...
impl crate::stitching::PyRotationWarperTrait for PyRotationWarper {
	fn as_raw_PyRotationWarper(&self) -> *mut c_void { self.ptr }
}

impl PyRotationWarper {
	pub fn new(typ: &str, scale: f32) -> Result<crate::stitching::PyRotationWarper> {
		string_arg!(typ);
		unsafe { sys::cv_PyRotationWarper_PyRotationWarper_String_float(typ.as_ptr() as _, scale) }.into_result().map(|ptr| crate::stitching::PyRotationWarper { ptr })
	}
...
pub trait StitcherTrait {
	...
	fn features_finder(&mut self) -> Result<types::PtrOfFeature2D> {
		unsafe { sys::cv_Stitcher_featuresFinder(self.as_raw_Stitcher()) }.into_result().map(|ptr| types::PtrOfFeature2D { ptr })
	}
	
	fn features_finder_1(&self) -> Result<types::PtrOfFeature2D> {
		unsafe { sys::cv_Stitcher_featuresFinder_const(self.as_raw_Stitcher()) }.into_result().map(|ptr| types::PtrOfFeature2D { ptr })
	}
	
	fn set_features_finder(&mut self, features_finder: types::PtrOfFeature2D) -> Result<()> {
		unsafe { sys::cv_Stitcher_setFeaturesFinder_Ptr_Feature2D_(self.as_raw_Stitcher(), features_finder.as_raw_PtrOfFeature2D()) }.into_result()
	}
	...
	fn estimator(&mut self) -> Result<types::PtrOfDetail_Estimator> {
		unsafe { sys::cv_Stitcher_estimator(self.as_raw_Stitcher()) }.into_result().map(|ptr| types::PtrOfDetail_Estimator { ptr })
	}
	
	fn estimator_1(&self) -> Result<types::PtrOfDetail_Estimator> {
		unsafe { sys::cv_Stitcher_estimator_const(self.as_raw_Stitcher()) }.into_result().map(|ptr| types::PtrOfDetail_Estimator { ptr })
	}
	
	fn set_estimator(&mut self, estimator: types::PtrOfDetail_Estimator) -> Result<()> {
		unsafe { sys::cv_Stitcher_setEstimator_Ptr_Estimator_(self.as_raw_Stitcher(), estimator.as_raw_PtrOfDetail_Estimator()) }.into_result()
	}
	...
	fn estimate_transform(&mut self, images: &dyn core::ToInputArray, masks: &dyn core::ToInputArray) -> Result<crate::stitching::Stitcher_Status> {
		input_array_arg!(images);
		input_array_arg!(masks);
		unsafe { sys::cv_Stitcher_estimateTransform_const__InputArrayX_const__InputArrayX(self.as_raw_Stitcher(), images.as_raw__InputArray(), masks.as_raw__InputArray()) }.into_result()
	}
	...
	fn stitch_mask(&mut self, images: &dyn core::ToInputArray, masks: &dyn core::ToInputArray, pano: &mut dyn core::ToOutputArray) -> Result<crate::stitching::Stitcher_Status> {
		...
		input_array_arg!(masks);
		output_array_arg!(pano);
		unsafe { sys::cv_Stitcher_stitch_const__InputArrayX_const__InputArrayX_const__OutputArrayX(self.as_raw_Stitcher(), images.as_raw__InputArray(), masks.as_raw__InputArray(), pano.as_raw__OutputArray()) }.into_result()
	}
...
impl Stitcher {
	...
	pub fn create(mode: crate::stitching::Stitcher_Mode) -> Result<types::PtrOfStitcher> {
		unsafe { sys::cv_Stitcher_create_Mode(mode) }.into_result().map(|ptr| types::PtrOfStitcher { ptr })
	}
...
impl Detail_AffineBasedEstimator {
	pub fn default() -> Result<crate::stitching::Detail_AffineBasedEstimator> {
		unsafe { sys::cv_detail_AffineBasedEstimator_AffineBasedEstimator() }.into_result().map(|ptr| crate::stitching::Detail_AffineBasedEstimator { ptr })
	}
...
impl Detail_BestOf2NearestMatcher {
	...
	/// * num_matches_thresh1: 6
	/// * num_matches_thresh2: 6
	pub fn create(try_use_gpu: bool, match_conf: f32, num_matches_thresh1: i32, num_matches_thresh2: i32) -> Result<types::PtrOfDetail_BestOf2NearestMatcher> {
		unsafe { sys::cv_detail_BestOf2NearestMatcher_create_bool_float_int_int(try_use_gpu, match_conf, num_matches_thresh1, num_matches_thresh2) }.into_result().map(|ptr| types::PtrOfDetail_BestOf2NearestMatcher { ptr })
	}
...
impl Drop for Detail_BlocksChannelsCompensator {
	fn drop(&mut self) {
		...
		unsafe { cv_Detail_BlocksChannelsCompensator_delete(self.as_raw_Detail_BlocksChannelsCompensator()) };
	}
}

impl Detail_BlocksChannelsCompensator {
	pub fn as_raw_Detail_BlocksChannelsCompensator(&self) -> *mut c_void { self.ptr }
...
impl crate::stitching::Detail_BlocksChannelsCompensatorTrait for Detail_BlocksChannelsCompensator {
	fn as_raw_Detail_BlocksChannelsCompensator(&self) -> *mut c_void { self.ptr }
}

impl crate::stitching::Detail_BlocksCompensator for Detail_BlocksChannelsCompensator {
	fn as_raw_Detail_BlocksCompensator(&self) -> *mut c_void { self.ptr }
}
...
impl Detail_BlocksChannelsCompensator {
	...
	pub fn new(bl_width: i32, bl_height: i32, nr_feeds: i32) -> Result<crate::stitching::Detail_BlocksChannelsCompensator> {
		unsafe { sys::cv_detail_BlocksChannelsCompensator_BlocksChannelsCompensator_int_int_int(bl_width, bl_height, nr_feeds) }.into_result().map(|ptr| crate::stitching::Detail_BlocksChannelsCompensator { ptr })
	}
...
pub trait Detail_BlocksCompensator: crate::stitching::Detail_ExposureCompensator {
	fn as_raw_Detail_BlocksCompensator(&self) -> *mut c_void;
	fn apply(&mut self, index: i32, corner: core::Point, image: &mut dyn core::ToInputOutputArray, mask: &dyn core::ToInputArray) -> Result<()> {
		...
		input_array_arg!(mask);
		unsafe { sys::cv_detail_BlocksCompensator_apply_int_Point_const__InputOutputArrayX_const__InputArrayX(self.as_raw_Detail_BlocksCompensator(), index, &corner, image.as_raw__InputOutputArray(), mask.as_raw__InputArray()) }.into_result()
	}
	
	fn get_mat_gains(&mut self, umv: &mut types::VectorOfMat) -> Result<()> {
		unsafe { sys::cv_detail_BlocksCompensator_getMatGains_vector_Mat_X(self.as_raw_Detail_BlocksCompensator(), umv.as_raw_VectorOfMat()) }.into_result()
	}
	
	fn set_mat_gains(&mut self, umv: &mut types::VectorOfMat) -> Result<()> {
		unsafe { sys::cv_detail_BlocksCompensator_setMatGains_vector_Mat_X(self.as_raw_Detail_BlocksCompensator(), umv.as_raw_VectorOfMat()) }.into_result()
	}
	
	fn set_nr_feeds(&mut self, nr_feeds: i32) -> Result<()> {
		unsafe { sys::cv_detail_BlocksCompensator_setNrFeeds_int(self.as_raw_Detail_BlocksCompensator(), nr_feeds) }.into_result()
	}
	
	fn get_nr_feeds(&mut self) -> Result<i32> {
		unsafe { sys::cv_detail_BlocksCompensator_getNrFeeds(self.as_raw_Detail_BlocksCompensator()) }.into_result()
	}
	
	fn set_block_size(&mut self, width: i32, height: i32) -> Result<()> {
		unsafe { sys::cv_detail_BlocksCompensator_setBlockSize_int_int(self.as_raw_Detail_BlocksCompensator(), width, height) }.into_result()
	}
	
	fn set_block_size_1(&mut self, size: core::Size) -> Result<()> {
		unsafe { sys::cv_detail_BlocksCompensator_setBlockSize_Size(self.as_raw_Detail_BlocksCompensator(), &size) }.into_result()
	}
	...
	fn set_nr_gains_filtering_iterations(&mut self, nr_iterations: i32) -> Result<()> {
		unsafe { sys::cv_detail_BlocksCompensator_setNrGainsFilteringIterations_int(self.as_raw_Detail_BlocksCompensator(), nr_iterations) }.into_result()
	}
...
pub trait Detail_BlocksGainCompensatorTrait: crate::stitching::Detail_BlocksCompensator {
	...
	fn get_mat_gains(&mut self, umv: &mut types::VectorOfMat) -> Result<()> {
		unsafe { sys::cv_detail_BlocksGainCompensator_getMatGains_vector_Mat_X(self.as_raw_Detail_BlocksGainCompensator(), umv.as_raw_VectorOfMat()) }.into_result()
	}
	
	fn set_mat_gains(&mut self, umv: &mut types::VectorOfMat) -> Result<()> {
		unsafe { sys::cv_detail_BlocksGainCompensator_setMatGains_vector_Mat_X(self.as_raw_Detail_BlocksGainCompensator(), umv.as_raw_VectorOfMat()) }.into_result()
	}
...
impl Detail_BlocksGainCompensator {
	...
	pub fn new_1(bl_width: i32, bl_height: i32, nr_feeds: i32) -> Result<crate::stitching::Detail_BlocksGainCompensator> {
		unsafe { sys::cv_detail_BlocksGainCompensator_BlocksGainCompensator_int_int_int(bl_width, bl_height, nr_feeds) }.into_result().map(|ptr| crate::stitching::Detail_BlocksGainCompensator { ptr })
	}
...
pub trait Detail_ChannelsCompensatorTrait: crate::stitching::Detail_ExposureCompensator {
	fn as_raw_Detail_ChannelsCompensator(&self) -> *mut c_void;
	fn apply(&mut self, index: i32, corner: core::Point, image: &mut dyn core::ToInputOutputArray, mask: &dyn core::ToInputArray) -> Result<()> {
		input_output_array_arg!(image);
		input_array_arg!(mask);
		unsafe { sys::cv_detail_ChannelsCompensator_apply_int_Point_const__InputOutputArrayX_const__InputArrayX(self.as_raw_Detail_ChannelsCompensator(), index, &corner, image.as_raw__InputOutputArray(), mask.as_raw__InputArray()) }.into_result()
	}
	
	fn get_mat_gains(&mut self, umv: &mut types::VectorOfMat) -> Result<()> {
		unsafe { sys::cv_detail_ChannelsCompensator_getMatGains_vector_Mat_X(self.as_raw_Detail_ChannelsCompensator(), umv.as_raw_VectorOfMat()) }.into_result()
	}
	
	fn set_mat_gains(&mut self, umv: &mut types::VectorOfMat) -> Result<()> {
		unsafe { sys::cv_detail_ChannelsCompensator_setMatGains_vector_Mat_X(self.as_raw_Detail_ChannelsCompensator(), umv.as_raw_VectorOfMat()) }.into_result()
	}
	
	fn set_nr_feeds(&mut self, nr_feeds: i32) -> Result<()> {
		unsafe { sys::cv_detail_ChannelsCompensator_setNrFeeds_int(self.as_raw_Detail_ChannelsCompensator(), nr_feeds) }.into_result()
	}
	
	fn get_nr_feeds(&mut self) -> Result<i32> {
		unsafe { sys::cv_detail_ChannelsCompensator_getNrFeeds(self.as_raw_Detail_ChannelsCompensator()) }.into_result()
	}
	
	fn gains(&self) -> Result<types::VectorOfScalar> {
		unsafe { sys::cv_detail_ChannelsCompensator_gains_const(self.as_raw_Detail_ChannelsCompensator()) }.into_result().map(|ptr| types::VectorOfScalar { ptr })
	}
...
impl Drop for Detail_ChannelsCompensator {
	fn drop(&mut self) {
		...
		unsafe { cv_Detail_ChannelsCompensator_delete(self.as_raw_Detail_ChannelsCompensator()) };
	}
}

impl Detail_ChannelsCompensator {
	pub fn as_raw_Detail_ChannelsCompensator(&self) -> *mut c_void { self.ptr }
...
impl crate::stitching::Detail_ChannelsCompensatorTrait for Detail_ChannelsCompensator {
	fn as_raw_Detail_ChannelsCompensator(&self) -> *mut c_void { self.ptr }
}
...
impl Detail_ChannelsCompensator {
	...
	pub fn new(nr_feeds: i32) -> Result<crate::stitching::Detail_ChannelsCompensator> {
		unsafe { sys::cv_detail_ChannelsCompensator_ChannelsCompensator_int(nr_feeds) }.into_result().map(|ptr| crate::stitching::Detail_ChannelsCompensator { ptr })
	}
...
pub trait Detail_DpSeamFinderTrait: crate::stitching::Detail_SeamFinder {
	...
	fn set_cost_function_1(&mut self, val: &str) -> Result<()> {
		string_arg!(val);
		unsafe { sys::cv_detail_DpSeamFinder_setCostFunction_String(self.as_raw_Detail_DpSeamFinder(), val.as_ptr() as _) }.into_result()
	}
...
impl Detail_DpSeamFinder {
	...
	pub fn new_1(cost_func: &str) -> Result<crate::stitching::Detail_DpSeamFinder> {
		...
		unsafe { sys::cv_detail_DpSeamFinder_DpSeamFinder_String(cost_func.as_ptr() as _) }.into_result().map(|ptr| crate::stitching::Detail_DpSeamFinder { ptr })
	}
...
pub trait Detail_ExposureCompensator {
	...
	fn get_mat_gains(&mut self, unnamed: &mut types::VectorOfMat) -> Result<()> {
		unsafe { sys::cv_detail_ExposureCompensator_getMatGains_vector_Mat_X(self.as_raw_Detail_ExposureCompensator(), unnamed.as_raw_VectorOfMat()) }.into_result()
	}
	
	fn set_mat_gains(&mut self, unnamed: &mut types::VectorOfMat) -> Result<()> {
		unsafe { sys::cv_detail_ExposureCompensator_setMatGains_vector_Mat_X(self.as_raw_Detail_ExposureCompensator(), unnamed.as_raw_VectorOfMat()) }.into_result()
	}
	
	fn set_update_gain(&mut self, b: bool) -> Result<()> {
		unsafe { sys::cv_detail_ExposureCompensator_setUpdateGain_bool(self.as_raw_Detail_ExposureCompensator(), b) }.into_result()
	}
	
	fn get_update_gain(&mut self) -> Result<bool> {
		unsafe { sys::cv_detail_ExposureCompensator_getUpdateGain(self.as_raw_Detail_ExposureCompensator()) }.into_result()
	}
...
pub trait Detail_GainCompensatorTrait: crate::stitching::Detail_ExposureCompensator {
	...
	fn get_mat_gains(&mut self, umv: &mut types::VectorOfMat) -> Result<()> {
		unsafe { sys::cv_detail_GainCompensator_getMatGains_vector_Mat_X(self.as_raw_Detail_GainCompensator(), umv.as_raw_VectorOfMat()) }.into_result()
	}
	
	fn set_mat_gains(&mut self, umv: &mut types::VectorOfMat) -> Result<()> {
		unsafe { sys::cv_detail_GainCompensator_setMatGains_vector_Mat_X(self.as_raw_Detail_GainCompensator(), umv.as_raw_VectorOfMat()) }.into_result()
	}
	
	fn set_nr_feeds(&mut self, nr_feeds: i32) -> Result<()> {
		unsafe { sys::cv_detail_GainCompensator_setNrFeeds_int(self.as_raw_Detail_GainCompensator(), nr_feeds) }.into_result()
	}
	
	fn get_nr_feeds(&mut self) -> Result<i32> {
		unsafe { sys::cv_detail_GainCompensator_getNrFeeds(self.as_raw_Detail_GainCompensator()) }.into_result()
	}
...
impl Detail_GainCompensator {
	pub fn default() -> Result<crate::stitching::Detail_GainCompensator> {
		unsafe { sys::cv_detail_GainCompensator_GainCompensator() }.into_result().map(|ptr| crate::stitching::Detail_GainCompensator { ptr })
	}
	
	pub fn new(nr_feeds: i32) -> Result<crate::stitching::Detail_GainCompensator> {
		unsafe { sys::cv_detail_GainCompensator_GainCompensator_int(nr_feeds) }.into_result().map(|ptr| crate::stitching::Detail_GainCompensator { ptr })
	}
...
impl Detail_GraphCutSeamFinder {
	...
	pub fn new_1(cost_type: &str, terminal_cost: f32, bad_region_penalty: f32) -> Result<crate::stitching::Detail_GraphCutSeamFinder> {
		...
		unsafe { sys::cv_detail_GraphCutSeamFinder_GraphCutSeamFinder_String_float_float(cost_type.as_ptr() as _, terminal_cost, bad_region_penalty) }.into_result().map(|ptr| crate::stitching::Detail_GraphCutSeamFinder { ptr })
	}
...
pub trait Detail_ImageFeaturesTrait {
	...
	fn get_keypoints(&mut self) -> Result<types::VectorOfKeyPoint> {
		unsafe { sys::cv_detail_ImageFeatures_getKeypoints(self.as_raw_Detail_ImageFeatures()) }.into_result().map(|ptr| types::VectorOfKeyPoint { ptr })
	}
...
pub trait Detail_MatchesInfoTrait {
	...
	fn get_matches(&mut self) -> Result<types::VectorOfDMatch> {
		unsafe { sys::cv_detail_MatchesInfo_getMatches(self.as_raw_Detail_MatchesInfo()) }.into_result().map(|ptr| types::VectorOfDMatch { ptr })
	}
	
	fn get_inliers(&mut self) -> Result<types::VectorOfu8> {
		unsafe { sys::cv_detail_MatchesInfo_getInliers(self.as_raw_Detail_MatchesInfo()) }.into_result().map(|ptr| types::VectorOfu8 { ptr })
	}
...
pub trait Detail_NoExposureCompensatorTrait: crate::stitching::Detail_ExposureCompensator {
	...
	fn get_mat_gains(&mut self, umv: &mut types::VectorOfMat) -> Result<()> {
		unsafe { sys::cv_detail_NoExposureCompensator_getMatGains_vector_Mat_X(self.as_raw_Detail_NoExposureCompensator(), umv.as_raw_VectorOfMat()) }.into_result()
	}
	
	fn set_mat_gains(&mut self, umv: &mut types::VectorOfMat) -> Result<()> {
		unsafe { sys::cv_detail_NoExposureCompensator_setMatGains_vector_Mat_X(self.as_raw_Detail_NoExposureCompensator(), umv.as_raw_VectorOfMat()) }.into_result()
	}
...
impl dyn Detail_SeamFinder + '_ {
	pub fn create_default(typ: i32) -> Result<types::PtrOfDetail_SeamFinder> {
		unsafe { sys::cv_detail_SeamFinder_createDefault_int(typ) }.into_result().map(|ptr| types::PtrOfDetail_SeamFinder { ptr })
	}
