...
pub trait AffineFeature2D :  {
    #[doc(hidden)] fn as_raw_AffineFeature2D(&self) -> *mut c_void;
    ...
    /// ## C++ default parameters
    /// * mask: noArray()
    fn detect(&mut self, image: &core::Mat, keypoints: &mut types::VectorOfElliptic_KeyPoint, mask: &core::Mat) -> Result<()> {
        unsafe { sys::cv_xfeatures2d_AffineFeature2D_detect_Mat_VectorOfElliptic_KeyPoint_Mat(self.as_raw_AffineFeature2D(), image.as_raw_Mat(), keypoints.as_raw_VectorOfElliptic_KeyPoint(), mask.as_raw_Mat()) }.into_result()
    }
    ...
    fn detect_and_compute(&mut self, image: &core::Mat, mask: &core::Mat, keypoints: &mut types::VectorOfElliptic_KeyPoint, descriptors: &mut core::Mat, use_provided_keypoints: bool) -> Result<()> {
        unsafe { sys::cv_xfeatures2d_AffineFeature2D_detectAndCompute_Mat_Mat_VectorOfElliptic_KeyPoint_Mat_bool(self.as_raw_AffineFeature2D(), image.as_raw_Mat(), mask.as_raw_Mat(), keypoints.as_raw_VectorOfElliptic_KeyPoint(), descriptors.as_raw_Mat(), use_provided_keypoints) }.into_result()
    }
    
}

impl<'a> AffineFeature2D + 'a {
...
/// [Trzcinski13a](https://docs.opencv.org/3.4.6/d0/de3/citelist.html#CITEREF_Trzcinski13a) and [Trzcinski13b](https://docs.opencv.org/3.4.6/d0/de3/citelist.html#CITEREF_Trzcinski13b).
/// 
/// ## Parameters
...
/// samples subfolder.
pub trait BoostDesc :  {
    #[doc(hidden)] fn as_raw_BoostDesc(&self) -> *mut c_void;
...
impl<'a> BoostDesc + 'a {
    ...
    pub fn create(desc: i32, use_scale_orientation: bool, scale_factor: f32) -> Result<types::PtrOfBoostDesc> {
        unsafe { sys::cv_xfeatures2d_BoostDesc_create_int_bool_float(desc, use_scale_orientation, scale_factor) }.into_result().map(|x| types::PtrOfBoostDesc { ptr: x })
    }
    
}

// boxed class cv::xfeatures2d::BriefDescriptorExtractor
/// Class for computing BRIEF descriptors described in [calon2010](https://docs.opencv.org/3.4.6/d0/de3/citelist.html#CITEREF_calon2010) .
/// 
/// ## Parameters
...
impl BriefDescriptorExtractor {
    ...
    pub fn create(bytes: i32, use_orientation: bool) -> Result<types::PtrOfBriefDescriptorExtractor> {
        unsafe { sys::cv_xfeatures2d_BriefDescriptorExtractor_create_int_bool(bytes, use_orientation) }.into_result().map(|x| types::PtrOfBriefDescriptorExtractor { ptr: x })
    }
    
}

// Generating impl for trait cv::xfeatures2d::DAISY (trait)
/// Class implementing DAISY descriptor, described in [Tola10](https://docs.opencv.org/3.4.6/d0/de3/citelist.html#CITEREF_Tola10)
/// 
/// ## Parameters
...
/// * use_orientation: sample patterns using keypoints orientation, disabled by default.
pub trait DAISY :  {
    #[doc(hidden)] fn as_raw_DAISY(&self) -> *mut c_void;
    /// ## Parameters
    ...
    fn compute(&mut self, image: &core::Mat, keypoints: &mut types::VectorOfKeyPoint, descriptors: &mut core::Mat) -> Result<()> {
        unsafe { sys::cv_xfeatures2d_DAISY_compute_Mat_VectorOfKeyPoint_Mat(self.as_raw_DAISY(), image.as_raw_Mat(), keypoints.as_raw_VectorOfKeyPoint(), descriptors.as_raw_Mat()) }.into_result()
    }
    
    fn compute_1(&mut self, images: &types::VectorOfMat, keypoints: &mut types::VectorOfVectorOfKeyPoint, descriptors: &mut types::VectorOfMat) -> Result<()> {
        unsafe { sys::cv_xfeatures2d_DAISY_compute_VectorOfMat_VectorOfVectorOfKeyPoint_VectorOfMat(self.as_raw_DAISY(), images.as_raw_VectorOfMat(), keypoints.as_raw_VectorOfVectorOfKeyPoint(), descriptors.as_raw_VectorOfMat()) }.into_result()
    }
    ...
    fn compute_2(&mut self, image: &core::Mat, roi: core::Rect, descriptors: &mut core::Mat) -> Result<()> {
        unsafe { sys::cv_xfeatures2d_DAISY_compute_Mat_Rect_Mat(self.as_raw_DAISY(), image.as_raw_Mat(), roi, descriptors.as_raw_Mat()) }.into_result()
    }
    
    /// ## Parameters
    /// * image: image to extract descriptors
    /// * descriptors: resulted descriptors array for all image pixels
    fn compute_3(&mut self, image: &core::Mat, descriptors: &mut core::Mat) -> Result<()> {
        unsafe { sys::cv_xfeatures2d_DAISY_compute_Mat_Mat(self.as_raw_DAISY(), image.as_raw_Mat(), descriptors.as_raw_Mat()) }.into_result()
    }
...
impl<'a> DAISY + 'a {
    ...
    /// * interpolation: true
    /// * use_orientation: false
    pub fn create(radius: f32, q_radius: i32, q_theta: i32, q_hist: i32, norm: i32, h: &core::Mat, interpolation: bool, use_orientation: bool) -> Result<types::PtrOfDAISY> {
        unsafe { sys::cv_xfeatures2d_DAISY_create_float_int_int_int_int_Mat_bool_bool(radius, q_radius, q_theta, q_hist, norm, h.as_raw_Mat(), interpolation, use_orientation) }.into_result().map(|x| types::PtrOfDAISY { ptr: x })
    }
...
impl crate::xfeatures2d::Elliptic_KeyPoint {
    pub fn as_raw_Elliptic_KeyPoint(&self) -> *mut c_void { self.ptr }
    pub unsafe fn from_raw_ptr(ptr: *mut c_void) -> Self {
        Elliptic_KeyPoint {
            ptr
        }
...
impl Elliptic_KeyPoint {
    ...
    pub fn new() -> Result<crate::xfeatures2d::Elliptic_KeyPoint> {
        unsafe { sys::cv_xfeatures2d_Elliptic_KeyPoint_Elliptic_KeyPoint() }.into_result().map(|x| crate::xfeatures2d::Elliptic_KeyPoint { ptr: x })
    }
    
    pub fn new_1(pt: core::Point2f, angle: f32, axes: core::Size, size: f32, si: f32) -> Result<crate::xfeatures2d::Elliptic_KeyPoint> {
        unsafe { sys::cv_xfeatures2d_Elliptic_KeyPoint_Elliptic_KeyPoint_Point2f_float_Size_float_float(pt, angle, axes, size, si) }.into_result().map(|x| crate::xfeatures2d::Elliptic_KeyPoint { ptr: x })
    }
    
}

// boxed class cv::xfeatures2d::FREAK
/// Class implementing the FREAK (*Fast Retina Keypoint*) keypoint descriptor, described in [AOV12](https://docs.opencv.org/3.4.6/d0/de3/citelist.html#CITEREF_AOV12) .
/// 
...
impl FREAK {
    ...
    pub fn create(orientation_normalized: bool, scale_normalized: bool, pattern_scale: f32, n_octaves: i32, selected_pairs: &types::VectorOfint) -> Result<types::PtrOfFREAK> {
        unsafe { sys::cv_xfeatures2d_FREAK_create_bool_bool_float_int_VectorOfint(orientation_normalized, scale_normalized, pattern_scale, n_octaves, selected_pairs.as_raw_VectorOfint()) }.into_result().map(|x| types::PtrOfFREAK { ptr: x })
    }
    
}

// boxed class cv::xfeatures2d::HarrisLaplaceFeatureDetector
/// Class implementing the Harris-Laplace feature detector as described in [Mikolajczyk2004](https://docs.opencv.org/3.4.6/d0/de3/citelist.html#CITEREF_Mikolajczyk2004).
#[allow(dead_code)]
...
impl crate::xfeatures2d::HarrisLaplaceFeatureDetector {
    pub fn as_raw_HarrisLaplaceFeatureDetector(&self) -> *mut c_void { self.ptr }
    pub unsafe fn from_raw_ptr(ptr: *mut c_void) -> Self {
...
impl HarrisLaplaceFeatureDetector {
    ...
    pub fn create(num_octaves: i32, corn_thresh: f32, dog_thresh: f32, max_corners: i32, num_layers: i32) -> Result<types::PtrOfHarrisLaplaceFeatureDetector> {
        unsafe { sys::cv_xfeatures2d_HarrisLaplaceFeatureDetector_create_int_float_float_int_int(num_octaves, corn_thresh, dog_thresh, max_corners, num_layers) }.into_result().map(|x| types::PtrOfHarrisLaplaceFeatureDetector { ptr: x })
    }
...
impl LATCH {
    ...
    pub fn create(bytes: i32, rotation_invariance: bool, half_ssd_size: i32, sigma: f64) -> Result<types::PtrOfLATCH> {
        unsafe { sys::cv_xfeatures2d_LATCH_create_int_bool_int_double(bytes, rotation_invariance, half_ssd_size, sigma) }.into_result().map(|x| types::PtrOfLATCH { ptr: x })
    }
    
}

// boxed class cv::xfeatures2d::LUCID
/// Class implementing the locally uniform comparison image descriptor, described in [LUCID](https://docs.opencv.org/3.4.6/d0/de3/citelist.html#CITEREF_LUCID)
/// 
...
impl LUCID {
    ...
    pub fn create(lucid_kernel: i32, blur_kernel: i32) -> Result<types::PtrOfLUCID> {
        unsafe { sys::cv_xfeatures2d_LUCID_create_int_int(lucid_kernel, blur_kernel) }.into_result().map(|x| types::PtrOfLUCID { ptr: x })
    }
    
}

// boxed class cv::xfeatures2d::MSDDetector
/// Class implementing the MSD (*Maximal Self-Dissimilarity*) keypoint detector, described in [Tombari14](https://docs.opencv.org/3.4.6/d0/de3/citelist.html#CITEREF_Tombari14).
/// 
...
impl MSDDetector {
    ...
    pub fn create(m_patch_radius: i32, m_search_area_radius: i32, m_nms_radius: i32, m_nms_scale_radius: i32, m_th_saliency: f32, m_k_nn: i32, m_scale_factor: f32, m_n_scales: i32, m_compute_orientation: bool) -> Result<types::PtrOfMSDDetector> {
        unsafe { sys::cv_xfeatures2d_MSDDetector_create_int_int_int_int_float_int_float_int_bool(m_patch_radius, m_search_area_radius, m_nms_radius, m_nms_scale_radius, m_th_saliency, m_k_nn, m_scale_factor, m_n_scales, m_compute_orientation) }.into_result().map(|x| types::PtrOfMSDDetector { ptr: x })
    }
...
///       as described in [KrulisLS16](https://docs.opencv.org/3.4.6/d0/de3/citelist.html#CITEREF_KrulisLS16).
...
/// [KrulisLS16](https://docs.opencv.org/3.4.6/d0/de3/citelist.html#CITEREF_KrulisLS16)
/// [BeecksUS10](https://docs.opencv.org/3.4.6/d0/de3/citelist.html#CITEREF_BeecksUS10)
pub trait PCTSignatures : core::Algorithm {
    #[doc(hidden)] fn as_raw_PCTSignatures(&self) -> *mut c_void;
    /// Computes signature of given image.
    /// ## Parameters
    /// * image: Input image of CV_8U type.
    /// * signature: Output computed signature.
    fn compute_signature(&self, image: &core::Mat, signature: &mut core::Mat) -> Result<()> {
        unsafe { sys::cv_xfeatures2d_PCTSignatures_computeSignature_const_Mat_Mat(self.as_raw_PCTSignatures(), image.as_raw_Mat(), signature.as_raw_Mat()) }.into_result()
    }
    ...
    fn get_sampling_points(&self) -> Result<types::VectorOfPoint2f> {
        unsafe { sys::cv_xfeatures2d_PCTSignatures_getSamplingPoints_const(self.as_raw_PCTSignatures()) }.into_result().map(|x| types::VectorOfPoint2f { ptr: x })
    }
    ...
    fn get_init_seed_indexes(&self) -> Result<types::VectorOfint> {
        unsafe { sys::cv_xfeatures2d_PCTSignatures_getInitSeedIndexes_const(self.as_raw_PCTSignatures()) }.into_result().map(|x| types::VectorOfint { ptr: x })
    }
...
impl<'a> PCTSignatures + 'a {
    ...
    pub fn create(init_sample_count: i32, init_seed_count: i32, point_distribution: i32) -> Result<types::PtrOfPCTSignatures> {
        unsafe { sys::cv_xfeatures2d_PCTSignatures_create_int_int_int(init_sample_count, init_seed_count, point_distribution) }.into_result().map(|x| types::PtrOfPCTSignatures { ptr: x })
    }
    ...
    pub fn create_1(init_sampling_points: &types::VectorOfPoint2f, init_seed_count: i32) -> Result<types::PtrOfPCTSignatures> {
        unsafe { sys::cv_xfeatures2d_PCTSignatures_create_VectorOfPoint2f_int(init_sampling_points.as_raw_VectorOfPoint2f(), init_seed_count) }.into_result().map(|x| types::PtrOfPCTSignatures { ptr: x })
    }
    ...
    pub fn create_2(init_sampling_points: &types::VectorOfPoint2f, init_cluster_seed_indexes: &types::VectorOfint) -> Result<types::PtrOfPCTSignatures> {
        unsafe { sys::cv_xfeatures2d_PCTSignatures_create_VectorOfPoint2f_VectorOfint(init_sampling_points.as_raw_VectorOfPoint2f(), init_cluster_seed_indexes.as_raw_VectorOfint()) }.into_result().map(|x| types::PtrOfPCTSignatures { ptr: x })
    }
    ...
    /// * border_thickness: 1
    pub fn draw_signature(source: &core::Mat, signature: &core::Mat, result: &mut core::Mat, radius_to_shorter_side_ratio: f32, border_thickness: i32) -> Result<()> {
        unsafe { sys::cv_xfeatures2d_PCTSignatures_drawSignature_Mat_Mat_Mat_float_int(source.as_raw_Mat(), signature.as_raw_Mat(), result.as_raw_Mat(), radius_to_shorter_side_ratio, border_thickness) }.into_result()
    }
...
pub trait PCTSignaturesSQFD : core::Algorithm {
    #[doc(hidden)] fn as_raw_PCTSignaturesSQFD(&self) -> *mut c_void;
    ...
    fn compute_quadratic_form_distance(&self, _signature0: &core::Mat, _signature1: &core::Mat) -> Result<f32> {
        unsafe { sys::cv_xfeatures2d_PCTSignaturesSQFD_computeQuadraticFormDistance_const_Mat_Mat(self.as_raw_PCTSignaturesSQFD(), _signature0.as_raw_Mat(), _signature1.as_raw_Mat()) }.into_result()
    }
...
impl<'a> PCTSignaturesSQFD + 'a {
    ...
    pub fn create(distance_function: i32, similarity_function: i32, similarity_parameter: f32) -> Result<types::PtrOfPCTSignaturesSQFD> {
        unsafe { sys::cv_xfeatures2d_PCTSignaturesSQFD_create_int_int_float(distance_function, similarity_function, similarity_parameter) }.into_result().map(|x| types::PtrOfPCTSignaturesSQFD { ptr: x })
    }
...
/// (SIFT) algorithm by D. Lowe [Lowe04](https://docs.opencv.org/3.4.6/d0/de3/citelist.html#CITEREF_Lowe04) .
#[allow(dead_code)]
pub struct SIFT {
    #[doc(hidden)] pub(crate) ptr: *mut c_void
}
...
impl SIFT {
    ...
    pub fn create(nfeatures: i32, n_octave_layers: i32, contrast_threshold: f64, edge_threshold: f64, sigma: f64) -> Result<types::PtrOfSIFT> {
        unsafe { sys::cv_xfeatures2d_SIFT_create_int_int_double_double_double(nfeatures, n_octave_layers, contrast_threshold, edge_threshold, sigma) }.into_result().map(|x| types::PtrOfSIFT { ptr: x })
    }
    
}

// Generating impl for trait cv::xfeatures2d::SURF (trait)
/// Class for extracting Speeded Up Robust Features from an image [Bay06](https://docs.opencv.org/3.4.6/d0/de3/citelist.html#CITEREF_Bay06) .
/// 
...
pub trait SURF :  {
    #[doc(hidden)] fn as_raw_SURF(&self) -> *mut c_void;
...
impl<'a> SURF + 'a {
    ...
    pub fn create(hessian_threshold: f64, n_octaves: i32, n_octave_layers: i32, extended: bool, upright: bool) -> Result<types::PtrOfSURF> {
        unsafe { sys::cv_xfeatures2d_SURF_create_double_int_int_bool_bool(hessian_threshold, n_octaves, n_octave_layers, extended, upright) }.into_result().map(|x| types::PtrOfSURF { ptr: x })
    }
    
}

// boxed class cv::xfeatures2d::StarDetector
/// The class implements the keypoint detector introduced by [Agrawal08](https://docs.opencv.org/3.4.6/d0/de3/citelist.html#CITEREF_Agrawal08), synonym of StarDetector. :
#[allow(dead_code)]
...
impl StarDetector {
    ...
    pub fn create(max_size: i32, response_threshold: i32, line_threshold_projected: i32, line_threshold_binarized: i32, suppress_nonmax_size: i32) -> Result<types::PtrOfStarDetector> {
        unsafe { sys::cv_xfeatures2d_StarDetector_create_int_int_int_int_int(max_size, response_threshold, line_threshold_projected, line_threshold_binarized, suppress_nonmax_size) }.into_result().map(|x| types::PtrOfStarDetector { ptr: x })
    }
...
/// using "Descriptor Learning Using Convex Optimisation" (DLCO) aparatus described in [Simonyan14](https://docs.opencv.org/3.4.6/d0/de3/citelist.html#CITEREF_Simonyan14).
/// 
/// ## Parameters
...
pub trait VGG :  {
    #[doc(hidden)] fn as_raw_VGG(&self) -> *mut c_void;
...
impl<'a> VGG + 'a {
    ...
    pub fn create(desc: i32, isigma: f32, img_normalize: bool, use_scale_orientation: bool, scale_factor: f32, dsc_normalize: bool) -> Result<types::PtrOfVGG> {
        unsafe { sys::cv_xfeatures2d_VGG_create_int_float_bool_bool_float_bool(desc, isigma, img_normalize, use_scale_orientation, scale_factor, dsc_normalize) }.into_result().map(|x| types::PtrOfVGG { ptr: x })
    }
