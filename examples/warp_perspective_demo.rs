...
fn help() {
	...
	println!();
	println!("Usage:\n{} [image_name -- Default data/right.jpg]", env::args().next().unwrap());
	println!();
	println!("Hot keys:");
...
fn main() -> Result<(), Box<dyn Error>> {
	...
	let original_image_rows = original_image.rows() as f32;
	{
		...
		roi_corners.push(Point::new((original_image_cols / 1.15) as i32, (original_image_rows / 3.32) as i32));
		roi_corners.push(Point::new((original_image_cols / 1.33) as i32, (original_image_rows / 1.1) as i32));
		roi_corners.push(Point::new((original_image_cols / 1.93) as i32, (original_image_rows / 1.36) as i32));
	}
	...
	highgui::set_mouse_callback(WINDOW_TITLE, Some(Box::new({
		...
		move |event, x, y, _flags| {
			{
				...
				if roi_corners.len() == 4 {
					for i in 0..4 {
						if event == highgui::EVENT_LBUTTONDOWN && (roi_corners.get(i).unwrap().x - x).abs() < 10 && (roi_corners.get(i).unwrap().y - y).abs() < 10 {
	...
	while !end_program {
		...
		if validation_needed.load(Ordering::Relaxed) && roi_corners_len == 4 {
			image = original_image.clone();
			{
				...
				dst_corners.set(1, Point::new(
					(roi_corners.get(0)? - roi_corners.get(1)?).norm().max(
						(roi_corners.get(2)? - roi_corners.get(3)?).norm()
					) as i32,
					0,
				))?;
				dst_corners.set(2, Point::new(
					(roi_corners.get(0)? - roi_corners.get(1)?).norm().max(
						(roi_corners.get(2)? - roi_corners.get(3)?).norm()
					) as i32,
					(roi_corners.get(1)? - roi_corners.get(2)?).norm().max(
						(roi_corners.get(3)? - roi_corners.get(0)?).norm()
					) as i32,
				))?;
				dst_corners.set(3, Point::new(
					0,
					(roi_corners.get(1)? - roi_corners.get(2)?).norm().max(
						(roi_corners.get(3)? - roi_corners.get(0)?).norm()
					) as i32,
